...
#define _RND(sz, r) (__builtin_constant_p(r) && !((r) & ((r) - 1)) ? (((sz) + (r) - 1) & -(r)) : (((sz) + (r) - 1) / (r) * (r)))
#define _RND4(x)	_RND((x), 4)
#define _RND8(x)	_RND((x), 8)
#define _RND128(x)	_RND((x), 128)
#define _RND256(x)	_RND((x), 256)
#define _RND512(x)	_RND((x), 512)

#define bitshift(x)	__ffs(x)
