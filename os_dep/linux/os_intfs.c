...
static int rtw_net_set_mac_address(struct net_device *pnetdev, void *p)
{
	...
	if(!padapter->bup )
	{
		...
		copy_mac_addr(padapter->eeprompriv.mac_addr, addr->sa_data);
		//copy_mac_addr(pnetdev->dev_addr, addr->sa_data);
...
int _netdev_if2_open(struct net_device *pnetdev)
{
	...
	if(!primary_padapter->bup || !primary_padapter->hw_init_completed)
	{
		_netdev_open(primary_padapter->pnetdev);
	}

	if(!padapter->bup && primary_padapter->bup &&
...
_adapter *rtw_drv_if2_init(_adapter *primary_padapter, void (*set_intf_ops)(struct _io_ops *pops))
{
	...
	copy_mac_addr(mac, primary_padapter->eeprompriv.mac_addr);

	if (is_broadcast_mac_addr(mac) ||
	    is_zero_mac_addr(mac))
	{
		copy_mac_addr(mac, (u8 []){0x00, 0xe0, 0x4c, 0x87, 0x11, 0x22});
	}
	else
	{
	...
	copy_mac_addr(padapter->eeprompriv.mac_addr, mac);
...
void netdev_br_init(struct net_device *netdev)
{
	...
	//if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE))
	{
...
#endif  // (LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 35))
		{
			...
			if (br_netdev) {
				copy_mac_addr(adapter->br_mac, br_netdev->dev_addr);
				dev_put(br_netdev);
			} else
...
static int _rtw_drv_register_netdev(_adapter *padapter, char *name)
{
	...
	copy_mac_addr(pnetdev->dev_addr, padapter->eeprompriv.mac_addr);
...
int	rtw_gw_addr_query(_adapter *padapter)
{
	...
	if(!res)
	{
		RTW_WL32(pmlmepriv->gw_ip, gw_addr);
		copy_mac_addr(pmlmepriv->gw_mac_addr, gw_mac);
