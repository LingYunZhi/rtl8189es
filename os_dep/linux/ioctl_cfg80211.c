...
static const struct ieee80211_channel rtw_2ghz_channels[] = {
	CHAN2G(1, 2412, 0),
	CHAN2G(2, 2417, 0),
	CHAN2G(3, 2422, 0),
	CHAN2G(4, 2427, 0),
...
static const struct ieee80211_channel rtw_5ghz_a_channels[] = {
...
struct cfg80211_bss *rtw_cfg80211_inform_bss(_adapter *padapter, struct wlan_network *pnetwork)
{
	...
	*(fctrl) = 0;
	...
	if (pnetwork->network.Reserved[0] == 1) { // WIFI_BEACON
		...
		copy_mac_addr(pwlanhdr->addr1, myid(&(padapter->eeprompriv)));
		SetFrameSubType(pbuf, WIFI_PROBERSP);
	}

	copy_mac_addr(pwlanhdr->addr2, pnetwork->network.MacAddress);
	copy_mac_addr(pwlanhdr->addr3, pnetwork->network.MacAddress);
...
void rtw_cfg80211_ibss_indicate_connect(_adapter *padapter)
{
	...
	bss = rtw_cfg80211_inform_bss(padapter, cur_network);
	cfg80211_ibss_joined(padapter->pnetdev, bss->bssid, bss->channel, GFP_ATOMIC);
}
				...
				struct key_params *params)
{
	...
	case WLAN_CIPHER_SUITE_SMS4:
		...
		if(pairwise == NL80211_KEYTYPE_PAIRWISE) {
			...
			copy_mac_addr((void*)param->sta_addr, (void*)mac_addr);
		} else {
...
#ifdef CONFIG_AP_MODE
		if(mac_addr)
			copy_mac_addr(param->sta_addr, (void*)mac_addr);
				    ...
				    const u8 *mac, struct station_info *sinfo)
{
	...
	)
	{
		...
		if (!mac_addr_equal(mac, cur_network->network.MacAddress)) {
...
#ifdef CONFIG_CONCURRENT_MODE
	if(pbuddy_mlmepriv && pbuddy_mlmepriv->LinkDetectInfo.bBusyTraffic)
	{
...
#ifdef CONFIG_CONCURRENT_MODE
	if (check_buddy_fwstate(padapter, _FW_UNDER_LINKING)) {
				 ...
				 struct cfg80211_connect_params *sme)
{
	...
	if(!(wdev_to_priv(padapter->rtw_wdev))->bandroid_scan)
	{
#ifdef CONFIG_P2P
		struct wifidirect_info *pwdinfo= &(padapter->wdinfo);
		if(rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE))
	...
	if (!rtw_set_802_11_connect(padapter, sme->bssid, &ndis_ssid)) {
		ret = -1;
		goto exit;
	}
				  ...
				  struct cfg80211_pmksa *pmksa)
{
	...
	if (is_zero_mac_addr(pmksa->bssid) )
	{
		return -EINVAL;
	}
	...
	{
		if (mac_addr_equal( psecuritypriv->PMKIDList[index].Bssid, pmksa->bssid))
	...
	if(!blInserted)
	{
		...
		copy_mac_addr(psecuritypriv->PMKIDList[psecuritypriv->PMKIDIndex].Bssid, pmksa->bssid);
				  ...
				  struct cfg80211_pmksa *pmksa)
{
	...
	for(index=0 ; index<NUM_PMKID_CACHE; index++)
	{
		...
		{ // BSSID is matched, the same AP => Remove this PMKID information and reset it.
			set_zero_mac_addr( psecuritypriv->PMKIDList[index].Bssid );
...
void rtw_cfg80211_indicate_sta_disassoc(_adapter *padapter, unsigned char *da, unsigned short reason)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr2, da);
	...
	pmgmt_frame = rtw_set_fixed_ie(pmgmt_frame, _RSON_CODE_ , reason, &frame_len);
...
static int rtw_add_beacon(_adapter *adapter, const u8 *head, size_t head_len, const u8 *tail, size_t tail_len)
{
	...
	rtw_ies_remove_ie(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, P2P_OUI);
	rtw_ies_remove_ie(pbuf, &len, _BEACON_IE_OFFSET_, _VENDOR_SPECIFIC_IE_, WFD_OUI);
			       ...
			       const u8 *mac)
{
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if (mac_addr_equal(mac, psta->hwaddr))
		{
...
void rtw_cfg80211_issue_p2p_provision_request(_adapter *padapter, const u8 *buf, size_t len)
{
	...
	copy_mac_addr(pwdinfo->tx_prov_disc_info.peerIFAddr, GetAddr1Ptr(buf));
	copy_mac_addr(pwdinfo->tx_prov_disc_info.peerDevAddr, GetAddr1Ptr(buf));
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, pwdinfo->tx_prov_disc_info.peerDevAddr);
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr3, pwdinfo->tx_prov_disc_info.peerDevAddr);
	...
	RTW_WL16(p2p_ie + p2pielen, 0x0002);
	p2pielen += 2;
	...
	RTW_WL16(p2p_ie + p2pielen, devinfo_contentlen);
	p2pielen += 2;
