	...
	)
{
	...
	if(pHalData->BoardType == BOARD_USB_High_PA || Adapter->proximity.proxim_support)
#else
...
#ifdef CONFIG_INTEL_PROXIM
	if(Adapter->proximity.proxim_on){
		...
		for(i=0; i< NUM_STA; i++)
		{
			...
			while (!rtw_end_of_queue_search(phead, plist))
			{
				...
				if(is_broadcast_mac_addr(psta->hwaddr) ||
					mac_addr_equal(psta->hwaddr, myid(&Adapter->eeprompriv)))
					continue;
	...
	)
{
	...
	if(*(pDM_Odm->pbPowerSaving)) //pHalData->eRFPowerState==eRfOff
	{
	...
	)
{
	...
	bTxPathSel = (pDM_Odm->RFType != ODM_1T1R);
...
//modify by Guo.Mingzhi 2011-12-29
		EDCA_BE=(*pbIsCurRDLState)?EDCA_BE_DL:EDCA_BE_UL;
