	...
	struct phy_stat *pphy_status)
{
	...
	pkt_info.bPacketMatchBSSID = ((!IsFrameTypeCtrl(wlanhdr)) &&
		...
		mac_addr_equal(get_hdr_bssid(wlanhdr), get_bssid(&padapter->mlmepriv)));

	pkt_info.bPacketToSelf = pkt_info.bPacketMatchBSSID && mac_addr_equal(get_da(wlanhdr), myid(&padapter->eeprompriv));
