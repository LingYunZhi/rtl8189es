...
efuse_phymap_to_logical(u8 * phymap, u16 _offset, u16 _size_byte, u8  *pbuf)
{
	...
	eFuseWord= rtw_malloc(EFUSE_MAX_SECTION_88E * EFUSE_MAX_WORD_UNIT * sizeof(u16));
	if(eFuseWord == NULL)
	{
	...
	memset(eFuseWord, -1, EFUSE_MAX_SECTION_88E * EFUSE_MAX_WORD_UNIT * sizeof(u16));
	...
	//
#ifndef __BIG_ENDIAN
	memcpy(efuseTbl, eFuseWord, EFUSE_MAX_SECTION_88E * EFUSE_MAX_WORD_UNIT * sizeof(u16));
#else
	for(i=0; i<EFUSE_MAX_SECTION_88E; i++)
	{
		for(j=0; j<EFUSE_MAX_WORD_UNIT; j++)
			RTW_WL16(efuseTbl + i * EFUSE_MAX_WORD_UNIT + j * 2, eFuseWord[i][j]);
	}
#endif
	...
	//
	memcpy(pbuf, efuseTbl + _offset, _size_byte);
...
exit:
	...
	if(eFuseWord)
		rtw_mfree((void *)eFuseWord, EFUSE_MAX_SECTION_88E * EFUSE_MAX_WORD_UNIT * sizeof(u16));
}
		...
		for(addr=0;addr< data_cnts;addr++){
			...
			do{
				...
				if(rstatus){
					...
					RTW_WN32(pbuf+(addr*8), fifo_data);

					fifo_data = rtw_read32(Adapter,REG_PKTBUF_DBG_DATA_H);
					//printk("fifo_data_148:0x%08x\n",fifo_data);
					RTW_WN32(pbuf+(addr*8+4), fifo_data);
				}
				rtw_usleep_os(2);
	...
	)
{
	...
	switch (remain) {
	case 1: pFwBuf[FwLen++] = 0;
	case 2: pFwBuf[FwLen++] = 0;
	case 3: pFwBuf[FwLen++] = 0;
	}
...
//	RT_TRACE(_module_hal_init_c_, _drv_err_, ("rtl8723a_FirmwareDownload: %s\n", pFwImageFileName));
	...
	#ifdef CONFIG_FILE_FWIMG
	if(rtw_is_file_readable(rtw_fw_file_path))
	{
	...
	)
{
	...
	eFuseWord= rtw_zmalloc(EFUSE_MAX_SECTION_88E * EFUSE_MAX_WORD_UNIT * sizeof(u16));
	if(eFuseWord == NULL)
	{
		DBG_871X("%s: alloc eFuseWord fail!\n", __FUNCTION__);
		goto exit;
	...
	)
{
	switch(type)
	{
		case TYPE_EFUSE_MAX_SECTION:
			return EFUSE_MAX_SECTION_88E;
		case TYPE_EFUSE_REAL_CONTENT_LEN:
			return EFUSE_REAL_CONTENT_LEN_88E;
		case TYPE_EFUSE_CONTENT_LEN_BANK:
			return EFUSE_REAL_CONTENT_LEN_88E;
		case TYPE_AVAILABLE_EFUSE_BYTES_BANK:
			return EFUSE_REAL_CONTENT_LEN_88E - EFUSE_OOB_PROTECT_BYTES_88E;
		case TYPE_AVAILABLE_EFUSE_BYTES_TOTAL:
			return EFUSE_REAL_CONTENT_LEN_88E - EFUSE_OOB_PROTECT_BYTES_88E;
		case TYPE_EFUSE_MAP_LEN:
			return EFUSE_MAP_LEN_88E;
		case TYPE_EFUSE_PROTECT_BYTES_BANK:
			return EFUSE_OOB_PROTECT_BYTES_88E;
		default:
			return 0;
	}
}
	...
	)
{
	if(bPseudoTest)
	{
		return Hal_EFUSEGetEfuseDefinition_Pseudo88E(pAdapter, efuseType, type);
	}
	else
	{
		return Hal_EFUSEGetEfuseDefinition88E(pAdapter, efuseType, type);
	}
}
...
Hal_EfuseWordEnableDataWrite_Pseudo(	IN	PADAPTER	pAdapter,
							...
							IN	BOOLEAN		bPseudoTest)
{
	return Hal_EfuseWordEnableDataWrite(pAdapter, efuse_addr, word_en, data, bPseudoTest);
}
...
Hal_EfuseGetCurrentSize_Pseudo(IN	PADAPTER	pAdapter,
		IN		BOOLEAN			bPseudoTest)
{
	return hal_EfuseGetCurrentSize_8188e(pAdapter, bPseudoTest);
}
	...
	IN	BOOLEAN		bPseudoTest)
{
	...
	max_section = EFUSE_GetEfuseDefinition(pAdapter, EFUSE_WIFI, TYPE_EFUSE_MAX_SECTION, bPseudoTest);
...
Hal_EfusePgPacketRead(	IN	PADAPTER	pAdapter,
					...
					IN	BOOLEAN			bPseudoTest)
{
	return hal_EfusePgPacketRead_8188e(pAdapter, offset, data, bPseudoTest);
}
	...
	IN			BOOLEAN			bPseudoTest)
{
	...
	u8	repeatcnt=EFUSE_REPEAT_THRESHOLD_;

	//RTPRINT(FEEPROM, EFUSE_PG, ("Wirte 2byte header\n"));
	efuse_max_available_len = EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_AVAILABLE_EFUSE_BYTES_BANK, bPseudoTest);
	...
	while(efuse_addr < efuse_max_available_len)
	{
		...
		for (repeatcnt = EFUSE_REPEAT_THRESHOLD_; repeatcnt > 0; repeatcnt--) {
...
)
{
	...
	match_word_en = (pTargetPkt->word_en | pCurPkt->word_en) & 0xf;
	*pWden = match_word_en;
					...
					)
{
	...
	efuse_max_available_len = EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_AVAILABLE_EFUSE_BYTES_BANK, bPseudoTest);
	efuse_max = EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_EFUSE_REAL_CONTENT_LEN, bPseudoTest);
	...
	while(1)
	{
		...
		if(efuse_OneByteRead(pAdapter, startAddr, &efuse_data, bPseudoTest) && (efuse_data!=0xFF))
		{
				...
				wordEnMatched(pTargetPkt, &curPkt, &matched_wden) )
			{
				...
				pTargetPkt->word_en |= (~matched_wden & 0xf);
	...
	)
{
	...
	efuse_max_available_len = EFUSE_GetEfuseDefinition(pAdapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN, _FALSE);

	//efuse_max_available_len = EFUSE_GetEfuseDefinition(pAdapter, efuseType, TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, bPseudoTest);
...
Hal_EfusePgPacketWrite_Pseudo(IN	PADAPTER	pAdapter,
					...
					IN	BOOLEAN		bPseudoTest)
{
	return hal_EfusePgPacketWrite_8188e(pAdapter, offset, word_en, data, bPseudoTest);
}
