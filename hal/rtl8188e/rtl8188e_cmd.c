	...
	*(fctrl) = 0;
	...
	switch(cur_network->network.InfrastructureMode)
	{
		case Ndis802_11Infrastructure:
			...
			copy_mac_addr(pwlanhdr->addr1, get_my_bssid(&(pmlmeinfo->network)));
			copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
			copy_mac_addr(pwlanhdr->addr3, StaAddr);
			break;
		case Ndis802_11APMode:
			SetFrDs(fctrl);
			copy_mac_addr(pwlanhdr->addr1, StaAddr);
			copy_mac_addr(pwlanhdr->addr2, get_my_bssid(&(pmlmeinfo->network)));
...
//YJ,del,120503
	...
	*pLength += 28;
	if (psecuritypriv->dot11PrivacyAlgrthm == _TKIP_)
	{
		...
		if (psta != NULL) {
			if(!memchr_inv(&psta->dot11tkiptxmickey.skey[0],0, 16)){
