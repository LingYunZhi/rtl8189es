...
void interrupt_handler_8188eu(_adapter *padapter,u16 pkt_len,u8 *pbuf)
{
	...
	#endif
	{
		...
		{
			//send_beacon(padapter);
			if(padapter->pbuddy_adapter->mlmepriv.update_bcn)
			{
				//tx_beacon_hdl(padapter, NULL);
...
static s32 pre_recv_entry(union recv_frame *precvframe, struct recv_stat *prxstat, struct phy_stat *pphy_status)
{
	...
	if(!IS_MCAST(paddr1) )//unicast packets
	{
		//primary_myid = myid(&primary_padapter->eeprompriv);
		secondary_myid = myid(&secondary_padapter->eeprompriv);

		if(mac_addr_equal(paddr1, secondary_myid))
		{
	...
	else // Handle BC/MC Packets
	{
		...
		switch (type)
		{
			case WIFI_MGT_TYPE: //Handle BC/MC mgnt Packets
				if(subtype == WIFI_BEACON)
				{
					...
					if (check_fwstate(&secondary_padapter->mlmepriv, _FW_LINKED) &&
						mac_addr_equal(paddr3, get_bssid(&secondary_padapter->mlmepriv)))
					{
						//change to secondary interface
						precvframe->u.hdr.adapter = secondary_padapter;
						clone = _FALSE;
					...
					if(check_fwstate(&primary_padapter->mlmepriv, _FW_LINKED) &&
						mac_addr_equal(paddr3, get_bssid(&primary_padapter->mlmepriv)))
					{
						if(!clone)
						{
							clone = _TRUE;
			...
			case WIFI_DATA_TYPE: //Handle BC/MC data Packets
				...
				if(mac_addr_equal(paddr2, get_bssid(&secondary_padapter->mlmepriv)))
				{
					//change to secondary interface
					precvframe->u.hdr.adapter = secondary_padapter;
					clone = _FALSE;
...
static void usb_read_port_complete(struct urb *purb, struct pt_regs *regs)
{
	...
	else
	{
		...
		if(rtw_inc_and_chk_continual_io_error(adapter_to_dvobj(padapter)) ){
			padapter->bSurpriseRemoved = _TRUE;
		}
...
void rtl8188eu_xmit_tasklet(void *priv)
{
	...
	while(1)
	{
		if (padapter->bDriverStopped || padapter->bSurpriseRemoved || padapter->bWritePortCancel)
		{
