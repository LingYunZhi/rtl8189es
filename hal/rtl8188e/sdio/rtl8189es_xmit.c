...
static void rtl8188e_cal_txdesc_chksum(struct tx_desc *ptxdesc)
{
	...
	unsigned long *ulPtr = (unsigned long *)ptxdesc;
	int i;
...
//		RT_TRACE(_module_hal_xmit_c_, _drv_notice_, ("%s: MGNT_FRAMETAG\n", __FUNCTION__));
		...
		if(padapter->proximity.proxim_on && pattrib->intel_proxim){
...
query_free_page:
	// check if hardware tx fifo page is enough
	if( !rtl8188es_query_tx_freepage(pri_padapter, pxmitbuf))
	{
		rtw_msleep_os(1);
...
//		if (padapter->pbuddy_adapter->bSurpriseRemoved ||
...
static s32 xmit_xmitframes(PADAPTER padapter, struct xmit_priv *pxmitpriv)
{
	...
	do {
		...
		else
		{
			...
			if (!_rtw_queue_empty(&ptxservq->sta_pending) )
			{
...
//			_enter_critical(&pframe_queue->lock, &irqL1);
			...
			while (!rtw_end_of_queue_search(frame_phead, frame_plist) )
			{
