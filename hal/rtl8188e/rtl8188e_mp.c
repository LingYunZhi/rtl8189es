...
void Hal_MPT_CCKTxPowerAdjust(PADAPTER Adapter, BOOLEAN bInCH14)
{
	...
	if (!bInCH14)
	{
		...
		{
			if ((CurrCCKSwingVal & 0xffff) == RTW_RL16(CCKSwingTable_Ch1_Ch13[i]))
			{
				CCKSwingIndex = i;
...
//					(rCCK0_TxFilter1+2), CurrCCKSwingVal, CCKSwingIndex));
		...
		//Write 0xa22 0xa23
		TempVal = RTW_RL16(CCKSwingTable_Ch1_Ch13[CCKSwingIndex]);

		//Write 0xa24 ~ 0xa27
		TempVal2 = RTW_RL32(&CCKSwingTable_Ch1_Ch13[CCKSwingIndex][2]);

		//Write 0xa28  0xa29
		TempVal3 = RTW_RL16(&CCKSwingTable_Ch1_Ch13[CCKSwingIndex][6]);
	}
	else
	{
		for (i = 0; i < CCK_TABLE_SIZE; i++)
		{
			if ((CurrCCKSwingVal & 0xffff) == RTW_RL16(CCKSwingTable_Ch14[i]))
			{
				CCKSwingIndex = i;
...
//					(rCCK0_TxFilter1+2), CurrCCKSwingVal, CCKSwingIndex));
		...
		//Write 0xa22 0xa23
		TempVal = RTW_RL16(CCKSwingTable_Ch14[CCKSwingIndex]);

		//Write 0xa24 ~ 0xa27
		TempVal2 = RTW_RL32(&CCKSwingTable_Ch14[CCKSwingIndex][2]);

		//Write 0xa28  0xa29
		TempVal3 = RTW_RL16(&CCKSwingTable_Ch14[CCKSwingIndex][6]);
	}
...
void Hal_MPT_CCKTxPowerAdjustbyIndex(PADAPTER pAdapter, BOOLEAN beven)
{
	...
	if (Action != 0)
	{
		...
		for (i = 0; i < CCK_TABLE_SIZE; i++)
		{
			...
			{
				if (TempCCk == RTW_RN32(&CCKSwingTable_Ch14[i][2]))
				{
					CCK_index_old = (u8) i;
...
//						rCCK0_TxFilter2, TempCCk, CCK_index_old, pHalData->bCCKinCH14));
			...
			{
				if (TempCCk == RTW_RN32(&CCKSwingTable_Ch1_Ch13[i][2]))
				{
					CCK_index_old = (u8) i;
