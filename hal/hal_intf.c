...
uint	 rtw_hal_init(_adapter *padapter)
{
	...
	if((padapter->pbuddy_adapter != NULL) && padapter->DualMacConcurrent
		&& (padapter->adapter_type == PRIMARY_ADAPTER))
	{
		if(padapter->pbuddy_adapter->hw_init_completed)
		{
...
s32	rtw_hal_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe)
{
	...
	copy_mac_addr(pmgntframe->attrib.ra, pwlanhdr->addr1);
