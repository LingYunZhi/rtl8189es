...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED|WIFI_ADHOC_MASTER_STATE))
	{
		...
		if (mac_addr_equal(&pmlmepriv->cur_network.network.MacAddress, bssid))
		{
			if (!check_fwstate(pmlmepriv, WIFI_STATION_STATE))
...
handle_tkip_countermeasure:
	...
	copy_mac_addr(&pmlmepriv->assoc_bssid, bssid);
...
_func_enter_;
	...
	if(key->KeyIndex & 0x40000000)
	{
		...
		// check BSSID
		if (is_broadcast_mac_addr(key->BSSID)){

			RT_TRACE(_module_rtl871x_ioctl_set_c_,_drv_err_,("is_broadcast_mac_addr(key->BSSID)\n"));
			ret= _FALSE;
			goto exit;
		}
	...
	else
	{
		...
		if(check_fwstate(&padapter->mlmepriv, WIFI_ADHOC_STATE) && !is_broadcast_mac_addr(key->BSSID)) {
		...
		if(check_fwstate(&padapter->mlmepriv, WIFI_ADHOC_STATE) && check_fwstate(&padapter->mlmepriv, _FW_LINKED))
		{
			bgrouptkey = _TRUE;
		}
	...
	if(key->KeyIndex & 0x20000000){
		...
		if(bgroup)
		{
			...
			RTW_WN64(&padapter->securitypriv.dot11Grprxpn, keysrc);
		}
		else
		{
			NDIS_802_11_KEY_RSC keysrc=key->KeyRSC & 0x00FFFFFFFFFFFFULL;
			RTW_WN64(&padapter->securitypriv.dot11Grptxpn, keysrc);
		}
