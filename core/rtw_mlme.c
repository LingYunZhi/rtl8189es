	...
	while (plist != phead)
       {
		...
		if (mac_addr_equal(addr, pnetwork->network.MacAddress))
                        break;
...
_func_exit_;
	...
	if ((feature == 1) && // 1: P2P supported
		mac_addr_equal(src->MacAddress, dst->MacAddress)
		) {
		return _TRUE;
	}
#endif
...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE))
	{
		...
		if(mac_addr_equal(&(pmlmepriv->cur_network.network.MacAddress), pnetwork->MacAddress))
		{
...
_func_enter_;
	...
	the_same_macaddr = mac_addr_equal(pnetwork->network.MacAddress, cur_network->network.MacAddress);
...
void rtw_dynamic_check_timer_handlder(_adapter *adapter)
{
	...
	if(pbuddy_adapter)
	{
		if(adapter->net_closed && pbuddy_adapter->net_closed)
		{
			return;
		}
	}
	...
	, struct wlan_network **candidate, struct wlan_network *competitor)
{
	...
	if(pmlmepriv->assoc_by_bssid) {
		if(!mac_addr_equal(competitor->network.MacAddress, pmlmepriv->assoc_bssid))
			goto exit;
	}
...
#if 0
	if(pmlmepriv->assoc_by_bssid) { // associate with bssid
		if(	(*candidate == NULL ||(*candidate)->network.Rssi<competitor->network.Rssi )
			&& mac_addr_equal(competitor->network.MacAddress, pmlmepriv->assoc_bssid)
		) {
			*candidate = competitor;
			updated = _TRUE;
		}
...
int rtw_restruct_wmm_ie(_adapter *adapter, u8 *in_ie, u8 *out_ie, uint in_len, uint initial_out_len)
{
	...
	for (i = 12; i<in_len; i+=(in_ie[i+1]+2))
	{
		...
		if(in_ie[i] == 0xDD && RTW_RN32(&in_ie[i+2]) == WMM_OUI && i+5 < in_len) //WMM element ID and OUI
		{
...
static int SecIsInPMKIDList(_adapter *Adapter, u8 *bssid)
{
	...
	do
	{
		if( ( psecuritypriv->PMKIDList[i].bUsed ) &&
                   mac_addr_equal( psecuritypriv->PMKIDList[i].Bssid, bssid ) )
		{
			break;
		}
		else
...
_func_enter_;
	...
	copy_mac_addr(pdev_network->MacAddress, myhwaddr);
...
void rtw_issue_addbareq_cmd(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	//if(bmcst || !padapter->mlmepriv.LinkDetectInfo.bTxBusyTraffic)
...
sint rtw_buddy_adapter_up(_adapter *padapter)
{
	...
	if(padapter->pbuddy_adapter == NULL)
	{
		...
		!padapter->pbuddy_adapter->bup || !padapter->pbuddy_adapter->hw_init_completed)
	{
		res = _FALSE;
	}
	else
