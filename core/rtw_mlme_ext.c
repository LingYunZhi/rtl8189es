...
static const struct mlme_handler mlme_ap_tbl[]={
	{WIFI_ASSOCREQ,		"OnAssocReq",	&OnAssocReq},
	{WIFI_ASSOCRSP,		"OnAssocRsp",	&OnAssocRsp},
	{WIFI_REASSOCREQ,	"OnReAssocReq",	&OnAssocReq},
	{WIFI_REASSOCRSP,	"OnReAssocRsp",	&OnAssocRsp},
...
static const struct action_handler OnAction_tbl[]={
...
***************************************************/
const unsigned char	WMM_INFO_IE[6] = {0x00, 0x50, 0xf2, 0x02, 0x00, 0x01};
const unsigned char	WMM_PARA_IE[6] = {0x00, 0x50, 0xf2, 0x02, 0x01, 0x01};
const unsigned char REALTEK_96B_IE[6] = {0x00, 0xe0, 0x4c, 0x02, 0x01, 0x20};

/********************************************************
MCS rate definitions
*********************************************************/
#ifdef CONFIG_DISABLE_MCS13TO15
const unsigned char	MCS_rate_2R_MCS13TO15_OFF[16] = {0xff, 0x1f, 0x0, 0x0, 0x01, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
const unsigned char	MCS_rate_2R[16] = {0xff, 0xff, 0x0, 0x0, 0x01, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
#else //CONFIG_DISABLE_MCS13TO15
const unsigned char	MCS_rate_2R[16] = {0xff, 0xff, 0x0, 0x0, 0x01, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};
#endif //CONFIG_DISABLE_MCS13TO15
const unsigned char	MCS_rate_1R[16] = {0xff, 0x00, 0x0, 0x0, 0x01, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0};

/********************************************************
ChannelPlan definitions
*********************************************************/
/*static const RT_CHANNEL_PLAN	DefaultChannelPlan[RT_CHANNEL_DOMAIN_MAX] = {
...
static const RT_CHANNEL_PLAN_2G	RTW_ChannelPlan2G[RT_CHANNEL_DOMAIN_2G_MAX] = {
...
static const RT_CHANNEL_PLAN_5G	RTW_ChannelPlan5G[RT_CHANNEL_DOMAIN_5G_MAX] = {
...
static const RT_CHANNEL_PLAN_MAP	RTW_ChannelPlanMap[RT_CHANNEL_DOMAIN_MAX] = {
...
static const RT_CHANNEL_PLAN_MAP	RTW_CHANNEL_PLAN_MAP_REALTEK_DEFINE = {0x03,0x02}; //use the conbination for max channel numbers
...
static void init_mlme_ext_priv_value(_adapter* padapter)
{
	...
	static const unsigned char	mixed_datarate[NumRates] = {_1M_RATE_, _2M_RATE_, _5M_RATE_, _11M_RATE_, _6M_RATE_,_9M_RATE_, _12M_RATE_, _18M_RATE_, _24M_RATE_, _36M_RATE_, _48M_RATE_, _54M_RATE_, 0xff};
	static const unsigned char	mixed_basicrate[NumRates] ={_1M_RATE_, _2M_RATE_, _5M_RATE_, _11M_RATE_, _6M_RATE_, _12M_RATE_, _24M_RATE_, 0xff,};
...
static void _mgt_dispatcher(_adapter *padapter, struct mlme_handler *ptable, union recv_frame *precv_frame)
{
	u8 *pframe = precv_frame->u.hdr.rx_data;
	  ...
	  if(ptable->func)
		...
		if (!mac_addr_equal(GetAddr1Ptr(pframe), myid(&padapter->eeprompriv)) &&
...
u32 p2p_listen_state_process(_adapter *padapter, unsigned char *da)
{
	...
	{
		if(!padapter->cfg80211_wdinfo.is_ro_ch
...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	   ...
	   )
	{
		...
		{
			if((is_valid_p2p_probereq = process_probe_req_p2p_ie(pwdinfo, pframe, len)))
			{
...
_continue:
	...
	//DBG_871X("+OnProbeReq\n");
		...
		if ((target_ie && (target_ielen == 4)) && !memcmp((void *)target_ie, "Ozmo",4)) {
			...
			copy_mac_addr(  pstapriv->atmel_rc_pattern, get_sa(pframe));
		}
#endif
...
unsigned int OnProbeRsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if(rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ))
	{
		...
		{
			if( mac_addr_equal( pwdinfo->tx_prov_disc_info.peerIFAddr, GetAddr2Ptr(pframe) ) )
			{
	...
	else if(rtw_p2p_chk_state(pwdinfo, P2P_STATE_GONEGO_ING))
	{
		if ( pwdinfo->nego_req_info.benable )
		{
			...
			if( mac_addr_equal( pwdinfo->nego_req_info.peerDevAddr, GetAddr2Ptr(pframe) ) )
			{
	...
	else if( rtw_p2p_chk_state(pwdinfo, P2P_STATE_TX_INVITE_REQ ) )
	{
		if ( pwdinfo->invitereq_info.benable )
		{
			...
			if( mac_addr_equal( pwdinfo->invitereq_info.peer_macaddr, GetAddr2Ptr(pframe) ) )
			{
	...
	if (mac_addr_equal(GetAddr3Ptr(pframe), get_my_bssid(&pmlmeinfo->network)))
	{
...
unsigned int OnAuth(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	while(plist != phead)
	{
		...
		plist = plist->next;
		if (mac_addr_equal((void *)sa, paclnode->addr)) {
...
unsigned int OnAuthClient(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (!mac_addr_equal(myid(&(padapter->eeprompriv)), get_da(pframe)))
		return _SUCCESS;
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	static const unsigned char WMM_IE[] = {0x00, 0x50, 0xf2, 0x02, 0x00, 0x01};
	...
	capab_info = RTW_RL16(pframe + WLAN_HDR_A3_LEN);
	//capab_info = le16_to_cpu(*(unsigned short *)(pframe + WLAN_HDR_A3_LEN));
	//listen_interval = le16_to_cpu(*(unsigned short *)(pframe + WLAN_HDR_A3_LEN+2));
	listen_interval = RTW_RL16(pframe + WLAN_HDR_A3_LEN+2);
	...
	if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
	{
		...
		if(rtw_get_wfd_ie(pframe + WLAN_HDR_A3_LEN + ie_offset , pkt_len - WLAN_HDR_A3_LEN - ie_offset , wfd_ie, &wfd_ielen ))
		{
			...
			if ( attr_contentlen )
			{
				pwdinfo->wfd_info->peer_rtsp_ctrlport = RTW_RB16( attr_content + 2 );
	...
	for (i = (6 + WLAN_HDR_A3_LEN); i < pkt_len;)
	{
		...
		switch (pIE->ElementID)
		{
			case _VENDOR_SPECIFIC_IE_:
				if (!memcmp(pIE->data, WMM_PARA_IE, 6))	//WMM
				{
					WMM_param_handler(padapter, pIE);
				}
#if defined(CONFIG_P2P) && defined(CONFIG_WFD)
				else if (RTW_RN32(pIE->data) == WFD_OUI)		//WFD
				{
...
unsigned int OnAction_back(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if (!mac_addr_equal(myid(&(padapter->eeprompriv)), GetAddr1Ptr(pframe)))//for if1, sta/ap mode
		return _SUCCESS;
		...
		switch (action)
		{
			...
			case RTW_WLAN_ACTION_DELBA: //DELBA
				if ((frame_body[3] & BIT(3)) == 0)
				{
					...
					reason_code = RTW_RL16(&frame_body[4]);
				}
...
void issue_p2p_GO_request(_adapter *padapter, u8* raddr)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, raddr);
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr3, myid(&(padapter->eeprompriv)));
	...
	pframe = rtw_set_fixed_ie(pframe, 4, P2P_OUI, &(pattrib->pktlen));
	...
	*(u16*) ( p2pie + p2pielen ) = cpu_to_le16( ETH_ALEN );
	...
	//	Value:
	copy_mac_addr( p2pie + p2pielen, myid( &padapter->eeprompriv ));
	p2pielen += ETH_ALEN;
...
void issue_p2p_GO_response(_adapter *padapter, u8* raddr, u8* frame_body,uint len, u8 result)
{
	...
	*(u16*) ( wpsie + wpsielen ) = cpu_to_be16( 0x0002 );
	...
	if (RTW_RN24(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req) == PACK_NE24('0','0','0'))
	{
		if ( wps_devicepassword_id == WPS_DPID_USER_SPEC )
		{
			RTW_WN24(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, PACK_NE24('d','i','s'));
		}
		else if ( wps_devicepassword_id == WPS_DPID_REGISTRAR_SPEC )
		{
			RTW_WN24(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, PACK_NE24('p','a','d'));
		}
		else
		{
			RTW_WN24(pwdinfo->rx_prov_disc_info.strconfig_method_desc_of_prov_disc_req, PACK_NE24('p','b','c'));
		}
	}
...
#ifdef CONFIG_CONCURRENT_MODE
	if (check_buddy_fwstate(padapter, _FW_LINKED))
		RTW_WL16A(p2pie + p2pielen, 5 + 1);
	else
#endif
		RTW_WL16A(p2pie + p2pielen, len_channellist_attr);
	p2pielen += 2;
	...
	*(u16*) ( p2pie + p2pielen ) = cpu_to_be16( pwdinfo->device_name_len );
	...
	if ( rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) )
	{
		...
		*(u16*) ( p2pie + p2pielen ) = cpu_to_le16( ETH_ALEN + pwdinfo->nego_ssidlen );
		...
		//	Value:
		//	p2P Device Address
		copy_mac_addr( p2pie + p2pielen , pwdinfo->device_addr);
		p2pielen += ETH_ALEN;
...
void issue_p2p_invitation_request(_adapter *padapter, u8* raddr )
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, raddr);
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr3, raddr);
	...
	*(u16*) ( p2pie + p2pielen ) = cpu_to_le16( 0x0005 );
	...
	if ( mac_addr_equal( myid( &padapter->eeprompriv ), pwdinfo->invitereq_info.go_bssid ) )
	{
		//	P2P Group BSSID
		//	Type:
		...
		*(u16*) ( p2pie + p2pielen ) = cpu_to_le16( ETH_ALEN );
		...
		copy_mac_addr( p2pie + p2pielen, pwdinfo->invitereq_info.go_bssid);
		p2pielen += ETH_ALEN;
	}
...
void issue_p2p_provision_request(_adapter *padapter, u8* pssid, u8 ussidlen, u8* pdev_raddr )
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, pdev_raddr);
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr3, pdev_raddr);
...
u8 is_matched_in_profilelist( u8* peermacaddr, struct profile_info* profileinfo )
{
	...
	for( i = 0; i < P2P_MAX_PERSISTENT_GROUP_NUM; i++, profileinfo++ )
	{
		...
		if ( mac_addr_equal( peermacaddr, profileinfo->peermac ) )
		{
			match_result = 1;
			DBG_871X( "[%s] Match!\n", __FUNCTION__ );
			break;
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	fctrl = &(pwlanhdr->frame_ctl);
	*(fctrl) = 0;
	copy_mac_addr(pwlanhdr->addr1, da);
	copy_mac_addr(pwlanhdr->addr2, mac);

	//	Use the device address for BSSID field.
	copy_mac_addr(pwlanhdr->addr3, mac);
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		*(u16*) ( wpsie + wpsielen ) = cpu_to_be16( 0x0010 );
		...
		if (pwdinfo->external_uuid == 0) {
			...
			copy_mac_addr( wpsie + wpsielen, myid( &padapter->eeprompriv ));
		} else {
		...
		*(u16*) ( wpsie + wpsielen ) = cpu_to_be16( ETH_ALEN );
		...
		//	Value:
		copy_mac_addr( wpsie + wpsielen, "123456" );
		wpsielen += ETH_ALEN;
...
int _issue_probereq_p2p(_adapter *padapter, u8 *da, int wait_ack)
{
	...
	*(fctrl) = 0;
	...
	if (da) {
		copy_mac_addr(pwlanhdr->addr1, da);
		copy_mac_addr(pwlanhdr->addr3, da);
	} else {
		if ( ( pwdinfo->p2p_info.scan_op_ch_only ) || ( pwdinfo->rx_invitereq_info.scan_op_ch_only ) )
		{
			//	This two flags will be set when this is only the P2P client mode.
			copy_mac_addr(pwlanhdr->addr1, pwdinfo->p2p_peer_interface_addr);
			copy_mac_addr(pwlanhdr->addr3, pwdinfo->p2p_peer_interface_addr);
		}
		else
		{
			//	broadcast probe request frame
			set_broadcast_mac_addr(pwlanhdr->addr1);
			set_broadcast_mac_addr(pwlanhdr->addr3);
		}
	}
	copy_mac_addr(pwlanhdr->addr2, mac);
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		switch( frame_body[ 6 ] )//OUI Subtype
		{
			case P2P_GO_NEGO_REQ:
			{
				...
				if( is_zero_mac_addr(pwdinfo->rx_prov_disc_info.peerDevAddr) );
					copy_mac_addr(pwdinfo->rx_prov_disc_info.peerDevAddr, GetAddr2Ptr(pframe));
				...
				if ( (p2p_ie=rtw_get_p2p_ie( frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)) )
				{
					...
					if ( attr_contentlen )
					{
						...
						if ( invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT )
						{
							...
							if ( attr_contentlen )
							{
								if ( mac_addr_equal( group_id.go_device_addr, myid( &padapter->eeprompriv ) ) )
								{
								...
								else
								{
									...
									else
									{
										rtw_p2p_set_state(pwdinfo, P2P_STATE_RECV_INVITE_REQ_DISMATCH );
										#ifdef CONFIG_INTEL_WIDI
										copy_mac_addr( pwdinfo->p2p_peer_device_addr, group_id.go_device_addr );
										rtw_p2p_set_role( pwdinfo, P2P_ROLE_CLIENT );
										#endif //CONFIG_INTEL_WIDI
			...
			case P2P_INVIT_RESP:
			{
				...
				if ( (p2p_ie=rtw_get_p2p_ie( frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)) )
				{
					...
					if ( attr_contentlen == 1 )
					{
						...
						{
							if ( mac_addr_equal( pwdinfo->invitereq_info.go_bssid, myid( &padapter->eeprompriv ) ))
							{
								rtw_p2p_set_role(pwdinfo, P2P_ROLE_GO );
							}
							else
...
unsigned int on_action_public_vendor(union recv_frame *precv_frame)
{
	...
	if (RTW_RN32(frame_body + 2) == P2P_OUI) {
...
unsigned int on_action_public(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	/* check RA matches or not */
	if (!mac_addr_equal(myid(&(padapter->eeprompriv)), GetAddr1Ptr(pframe)))
		goto exit;
...
void issue_beacon(_adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	set_broadcast_mac_addr(pwlanhdr->addr1);
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pwlanhdr->addr3, get_my_bssid(cur_network));
...
void issue_probersp(_adapter *padapter, unsigned char *da, u8 is_valid_p2p_probereq)
{
	...
	*(fctrl) = 0;
	copy_mac_addr(pwlanhdr->addr1, da);
	copy_mac_addr(pwlanhdr->addr2, mac);
	copy_mac_addr(pwlanhdr->addr3, bssid);
...
void issue_auth(_adapter *padapter, struct sta_info *psta, unsigned short status)
{
	...
	*(fctrl) = 0;
	...
	if(psta)// for AP mode
	{
		...
		copy_mac_addr(pwlanhdr->addr1, psta->hwaddr);
		copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
		copy_mac_addr(pwlanhdr->addr3, myid(&(padapter->eeprompriv)));
		...
		pframe = rtw_set_fixed_ie(pframe, _AUTH_ALGM_NUM_, val16, &(pattrib->pktlen));

		// setting auth seq number
		val16 =(u16)psta->auth_seq;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_ie(pframe, _AUTH_SEQ_NUM_, val16, &(pattrib->pktlen));

		// setting status code...
		val16 = status;
		val16 = cpu_to_le16(val16);
		pframe = rtw_set_fixed_ie(pframe, _STATUS_CODE_, val16, &(pattrib->pktlen));
	...
	{
		copy_mac_addr(pwlanhdr->addr1, get_my_bssid(&pmlmeinfo->network));
		copy_mac_addr(pwlanhdr->addr2, myid(&padapter->eeprompriv));
		copy_mac_addr(pwlanhdr->addr3, get_my_bssid(&pmlmeinfo->network));
		...
		if ((pmlmeinfo->auth_seq == 3) && (pmlmeinfo->state & WIFI_FW_AUTH_STATE) && (use_shared_key==1))
		{
			...
			pframe = rtw_set_fixed_ie(pframe, 4, val32, &(pattrib->pktlen));
...
void issue_asocrsp(_adapter *padapter, unsigned short status, struct sta_info *pstat, int pkt_type)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr((void *)GetAddr1Ptr(pwlanhdr), pstat->hwaddr);
	copy_mac_addr((void *)GetAddr2Ptr(pwlanhdr), myid(&(padapter->eeprompriv)));
	copy_mac_addr((void *)GetAddr3Ptr(pwlanhdr), get_my_bssid(&(pmlmeinfo->network)));
	...
	pframe = rtw_set_fixed_ie(pframe , _STATUS_CODE_ , status, &(pattrib->pktlen));

	val = cpu_to_le16(pstat->aid | BIT(14) | BIT(15));
	pframe = rtw_set_fixed_ie(pframe, _ASOC_ID_ , val, &(pattrib->pktlen));
...
void issue_assocreq(_adapter *padapter)
{
	...
	*(fctrl) = 0;
	...
	for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;)
	{
		...
		{
			case _VENDOR_SPECIFIC_IE_:
				if (RTW_RN32(pIE->data) == WPA_OUI ||
					RTW_RN32(pIE->data) == WMM_OUI ||
					RTW_RN32(pIE->data) == WPS_OUI)
				{
...
#endif //CONFIG_IOCTL_CFG80211
	{
		if(!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) && !rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE))
		{
			...
			*(u16*) ( p2pie + p2pielen ) = cpu_to_le16( 0x000D );
			...
			//	Value:
			copy_mac_addr( p2pie + p2pielen, pwdinfo->device_addr);	//	P2P Device Address
			p2pielen += ETH_ALEN;

			p2pie[ p2pielen++ ] = 1;	//	P2P Interface Address Count

			copy_mac_addr( p2pie + p2pielen, pwdinfo->device_addr);	//	P2P Interface Address List
			p2pielen += ETH_ALEN;
...
static int _issue_deauth(_adapter *padapter, unsigned char *da, unsigned short reason, u8 wait_ack)
{
	...
	*(fctrl) = 0;
	...
	pframe = rtw_set_fixed_ie(pframe, _RSON_CODE_ , reason, &(pattrib->pktlen));
...
void issue_action_spct_ch_switch(_adapter *padapter, u8 *ra, u8 new_ch, u8 ch_offset)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, ra); /* RA */
	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv))); /* TA */
	copy_mac_addr(pwlanhdr->addr3, ra); /* DA = RA */
...
void issue_action_SA_Query(_adapter *padapter, unsigned char *raddr, unsigned char action, unsigned short tid)
{
	...
	*(fctrl) = 0;
	...
	{
		case 0: //SA Query req
			pframe = rtw_set_fixed_ie(pframe, 2, pmlmeext->sa_query_seq, &pattrib->pktlen);
		...
		case 1: //SA Query rsp
			tid = cpu_to_le16(tid);
			pframe = rtw_set_fixed_ie(pframe, 2, tid, &pattrib->pktlen);
			break;
		default:
			break;
	}
	...
	if (category == 3)
	{
		switch (action)
		{
			case 0: //ADDBA req
				...
				pframe = rtw_set_fixed_ie(pframe, 2, BA_para_set, &(pattrib->pktlen));

				//BA_timeout_value = 0xffff;//max: 65535 TUs(~ 65 ms)
				BA_timeout_value = 5000;//~ 5ms
				BA_timeout_value = cpu_to_le16(BA_timeout_value);
				pframe = rtw_set_fixed_ie(pframe, 2, BA_timeout_value, &(pattrib->pktlen));
				...
				pframe = rtw_set_fixed_ie(pframe, 2, BA_starting_seqctrl, &(pattrib->pktlen));
				break;

			case 1: //ADDBA rsp
				pframe = rtw_set_fixed_ie(pframe, 1, pmlmeinfo->ADDBA_req.dialog_token, &(pattrib->pktlen));
				pframe = rtw_set_fixed_ie(pframe, 2, status, &(pattrib->pktlen));
				/*
				...
				*/
				...
				pframe = rtw_set_fixed_ie(pframe, 2, BA_para_set, &(pattrib->pktlen));
				pframe = rtw_set_fixed_ie(pframe, 2, pmlmeinfo->ADDBA_req.BA_timeout_value, &(pattrib->pktlen));
				break;
			case 2://DELBA
				...
				pframe = rtw_set_fixed_ie(pframe, 2, reason_code, &(pattrib->pktlen));
				break;
			default:
				break;
		}
...
void site_survey(_adapter *padapter)
{
	...
	if(survey_channel != 0)
	{
		...
		if( stay_buddy_ch == 1 )
		{
			...
			{
				update_beacon(padapter->pbuddy_adapter, 0, 0, _TRUE);
			}
		}
...
#ifdef CONFIG_CONCURRENT_MODE
				if(is_client_associated_to_ap(padapter->pbuddy_adapter))
				{
...
#ifdef CONFIG_CONCURRENT_MODE
			else if(is_client_associated_to_ap(padapter->pbuddy_adapter))
			{
				issue_nulldata(padapter->pbuddy_adapter, NULL, 0, 3, 500);
			}
#endif
...
u8 collect_bss_info(_adapter *padapter, union recv_frame *precv_frame, WLAN_BSSID_EX *bssid)
{
	...
	if (subtype == WIFI_PROBEREQ) {
		...
		copy_mac_addr(bssid->MacAddress, GetAddr2Ptr(pframe));
		bssid->Privacy = 1;
		return _SUCCESS;
	}
#endif //CONFIG_P2P
	...
	else
	{
		...
		copy_mac_addr(bssid->MacAddress, GetAddr3Ptr(pframe));
	}
...
void start_clnt_join(_adapter* padapter)
{
	...
	if (caps&cap_ESS)
	{
		...
		//	For the Win8 P2P connection, it will be hard to have a successful connection if this Wi-Fi doesn't connect to it.
		{
			...
			if (scanned == NULL || rtw_end_of_queue_search(head, pos) || !has_p2p_ie)
			#endif /* CONFIG_P2P */
...
unsigned int receive_disconnect(_adapter *padapter, unsigned char *MacAddr, unsigned short reason)
{
	...
	//check A3
	if (!mac_addr_equal(MacAddr, get_my_bssid(&pmlmeinfo->network)))
		return _SUCCESS;
...
void report_del_sta_event(_adapter *padapter, unsigned char* MacAddr, unsigned short reason)
{
	...
	copy_mac_addr((unsigned char *)(&(pdel_sta_evt->macaddr)), MacAddr);
	RTW_WN16(pdel_sta_evt->rsvd, reason);
...
void report_add_sta_event(_adapter *padapter, unsigned char* MacAddr, int cam_idx)
{
	...
	copy_mac_addr((unsigned char *)(&(padd_sta_evt->macaddr)), MacAddr);
...
void mlmeext_joinbss_event_callback(_adapter *padapter, int join_res)
{
	...
	if(join_res < 0)
	{
		join_type = 1;
		rtw_hal_set_hwreg(padapter, HW_VAR_MLME_JOIN, (u8 *)(&join_type));
		rtw_hal_set_hwreg(padapter, HW_VAR_BSSID, zero_mac_addr);
...
u8 setkey_hdl(_adapter *padapter, u8 *pbuf)
{
    ...
    padapter->HalFunc->SetHwRegHandler(padapter, HW_VAR_ON_RCR_AM, zero_mac_addr);
...
void dc_handle_join_done(_adapter *padapter, u8 join_res)
{
		...
		padapter->DualMacConcurrent)
	{
				...
				check_fwstate(pbuddy_mlmepriv, _FW_LINKED))
		{
			...
			update_beacon(pbuddy_adapter, 0, 0, _TRUE);
		}
		else	 if (is_client_associated_to_ap(pbuddy_adapter))
		{
			if((pbuddy_mlmeext->cur_bwmode == HT_CHANNEL_WIDTH_40) &&
...
u8 dc_handle_site_survey(_adapter *padapter)
{
	...
	if(pbuddy_adapter != NULL &&
		padapter->DualMacConcurrent)
	{
		if (is_client_associated_to_ap(pbuddy_adapter))
		{
			pmlmeext->sitesurvey_res.state = SCAN_TXNULL;
...
u8	dc_check_xmit(_adapter *padapter)
{
	...
	{
		pbuddy_mlmepriv = &(pbuddy_adapter->mlmepriv);
		if (check_fwstate(pbuddy_mlmepriv, _FW_UNDER_SURVEY|_FW_UNDER_LINKING))
		{
...
int rtw_chk_start_clnt_join(_adapter *padapter, u8 *ch, u8 *bw, u8 *offset)
{
	...
	if((pbuddy_pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE)//for AP MODE
	{
		...
		check_fwstate(pbuddy_mlmepriv, WIFI_STATION_STATE)) //for Client Mode/p2p client
	{
		DBG_871X("start_clnt_join: "ADPT_FMT"(ch=%d, bw=%d, ch_offset=%d)"
