...
_func_enter_;
	...
	set_zero_mac_addr(pstapriv->atmel_rc_pattern);
#endif
...
_func_enter_;
	...
	else
	{
		...
		copy_mac_addr(psta->hwaddr, hwaddr);
...
_func_enter_;
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if (mac_addr_equal(psta->hwaddr, addr))
...
struct sta_info* rtw_get_bcmc_stainfo(_adapter* padapter)
{
	...
	struct sta_priv 	*pstapriv = &padapter->stapriv;
_func_enter_;
	 psta = rtw_get_stainfo(pstapriv, broadcast_mac_addr);
_func_exit_;
	return psta;
...
u8 rtw_access_ctrl(_adapter *padapter, u8 *mac_addr)
{
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if(mac_addr_equal(paclnode->addr, mac_addr))
		{
			if(paclnode->valid)
			{
				match = _TRUE;
