	...
	if((pattrib->encrypt==_WEP40_)||(pattrib->encrypt==_WEP104_))
	{
		...
		for(curfragnum=0;curfragnum<pattrib->nr_frags;curfragnum++)
		{
			...
			else
			{
				...
				pframe=(u8 *)_RND4((SIZE_PTR)(pframe));
			}
		}
	}
...
_func_enter_;
	...
	if((prxattrib->encrypt==_WEP40_)||(prxattrib->encrypt==_WEP104_))
	{
		...
		crc = getcrc32(payload,length-4);

		if (crc != RTW_RL32(&payload[length - 4]))
		{
			RT_TRACE(_module_rtl871x_security_c_,_drv_err_,("rtw_wep_decrypt:icv error crc(%08x)!=payload[length-4](%08x)\n",
						crc, RTW_RL32(&payload[length - 4])));
		}
	}
...
void rtw_secmicsetkey(struct mic_data *pmicdata, u8 * key )
{
	// Set the key
_func_enter_;
	pmicdata->K0 = RTW_RL32( key );
	pmicdata->K1 = RTW_RL32( key + 4 );
...
_func_enter_;
	...
	RTW_WL32( dst, pmicdata->L );
	RTW_WL32( dst+4, pmicdata->R );
...
#define RotR1(v16)   (((u16)(v16) >> 1) | ((u16)(v16) << 15))
...
static const u16 Sbox1[2][256]=       /* Sbox for hash (can be in ROM)     */
{ {
...
/* 128 bit data.                        */
/****************************************/
static inline void xor_128(u8 *a, u8 *b, u8 *out)
{
    int i;
_func_enter_;
    for (i = 0; i < 16 / sizeof(long); i++)
    {
        RTW_WNLONG(&out[i * sizeof(long)],
			RTW_RNLONG(&a[i * sizeof(long)]) ^ RTW_RNLONG(&b[i * sizeof(long)]));
    }
_func_exit_;
}


static inline void xor_32(u8 *a, u8 *b, u8 *out)
{
_func_enter_;
	RTW_WN32(out, RTW_RN32(a) ^ RTW_RN32(b));
_func_exit_;
}


static inline u8 sbox(u8 a)
{
...
//	/*static*/ unsigned char	message[MAX_MSG_SIZE];
	...
	u8 chain_buffer[16] = { 0 };
	u8 aes_out[16] = { 0 };
	u8 padded_buffer[16] = { 0 };
	u8 mic[8];
//	uint	offset = 0;
...
_func_enter_;
    ...
    xor_128(aes_out, mic_header1, chain_buffer);
    aes128k128d(key, chain_buffer, aes_out);
    xor_128(aes_out, mic_header2, chain_buffer);
    aes128k128d(key, chain_buffer, aes_out);

	for (i = 0; i < num_blocks; i++)
    {
        xor_128(aes_out, &pframe[payload_index], chain_buffer);
        payload_index += 16;
        aes128k128d(key, chain_buffer, aes_out);
    }
    ...
    if (payload_remainder > 0)
    {
        ...
        xor_128(aes_out, padded_buffer, chain_buffer);
        aes128k128d(key, chain_buffer, aes_out);
    }
...
_func_enter_;
	...
	for (i = 0; i < num_blocks; i++)
    {
        xor_128(aes_out, &message[payload_index], chain_buffer);
...
u32	rtw_BIP_verify(_adapter *padapter, u8 *precvframe)
{
	...
	if(p)
	{
		...
		RTW_WN64(p+2+len-8, 0);

		//conscruct AAD, copy frame control field
		RTW_WN16(BIP_AAD, pwlanhdr->frame_ctl);
		ClearRetry(BIP_AAD);
		ClearPwrMgt(BIP_AAD);
		ClearMData(BIP_AAD);
		...
		if (RTW_RN64(mic) == RTW_RN64(pframe + pattrib->pkt_len - 8))
		{
...
static int sha256_done(struct sha256_state *md, unsigned char *out)
{
	...
	/* store length */
	RTW_WB64(md->buf + 56, md->length);
	sha256_compress(md, md->buf);

	/* copy output */
	for (i = 0; i < 8; i++)
		RTW_WB32(out + (4 * i), md->state[i]);
...
{
	unsigned long k_pad[64 / sizeof(long)]; /* padding - key XORd with ipad/opad */
	unsigned long tk[32 / sizeof(long)];
	u8 *_addr[6];
	size_t _len[6], i;
		...
		u8 *data, size_t data_len, u8 *buf, size_t buf_len)
{
	...
	RTW_WL16(length_le, buf_len * 8);
	pos = 0;
	while (pos < buf_len) {
		plen = buf_len - pos;
		RTW_WL16(counter_le, counter);
...
static void rijndaelKeySetupEnc(u32 rk[/*44*/], const u8 cipherKey[])
{
	...
	rk[0] = RTW_RB32(cipherKey     );
	rk[1] = RTW_RB32(cipherKey +  4);
	rk[2] = RTW_RB32(cipherKey +  8);
	rk[3] = RTW_RB32(cipherKey + 12);
	for (i = 0; i < 10; i++) {
		temp  = rk[3];
		rk[4] = rk[0] ^
...
static void rijndaelEncrypt(u32 rk[/*44*/], u8 pt[16], u8 ct[16])
{
	 ...
	 */
	s0 = RTW_RB32(pt     ) ^ rk[0];
	s1 = RTW_RB32(pt +  4) ^ rk[1];
	s2 = RTW_RB32(pt +  8) ^ rk[2];
	s3 = RTW_RB32(pt + 12) ^ rk[3];
...
void wpa_tdls_generate_tpk(_adapter *padapter, struct sta_info *psta)
{
	...
	if (memcmp(myid(&(padapter->eeprompriv)), psta->hwaddr, ETH_ALEN) < 0) {
		copy_mac_addr(data, myid(&(padapter->eeprompriv)));
		copy_mac_addr(data + ETH_ALEN, psta->hwaddr);
	} else {
		copy_mac_addr(data, psta->hwaddr);
		copy_mac_addr(data + ETH_ALEN, myid(&(padapter->eeprompriv)));
	}
	copy_mac_addr(data + 2 * ETH_ALEN, get_bssid(pmlmepriv));
							...
							u8 *mic)
{
	...
	copy_mac_addr(pos, _lnkid->init_sta);
	pos += ETH_ALEN;
	/* 2) TDLS responder STA MAC address */
	copy_mac_addr(pos, _lnkid->resp_sta);
	pos += ETH_ALEN;
							...
							u8 *lnkid, u8 *rsnie, u8 *timeoutie, u8 *ftie)
{
	...
	pos = buf;
	/* 1) TDLS initiator STA MAC address */
	copy_mac_addr(pos, lnkid + ETH_ALEN + 2);
	pos += ETH_ALEN;
	/* 2) TDLS responder STA MAC address */
	copy_mac_addr(pos, lnkid + 2 * ETH_ALEN + 2);
	pos += ETH_ALEN;
	/* 3) Transaction Sequence number */
	*pos++ = trans_seq;
	/* 4) Link Identifier IE */
