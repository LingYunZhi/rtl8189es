	...
	)
{
	...
	*pTemp &= 0x000f;
	...
	copy_mac_addr((TempBuf+16), (pHeader+16)); //Addr3
	...
	{
		copy_mac_addr(TempBuf+22, pHeader+24);
		QosOffset = 30;
	}else{
		set_zero_mac_addr(TempBuf+22);
		QosOffset = 24;
	}
...
)
{
	...
	if (!mac_addr_equal(pRaddr, padapter->pnetdev->dev_addr, ETH_ALEN))
		&&	! (pDaddr)
...
int SecSMS4HeaderFillIV(_adapter *padapter, u8 *pxmitframe)
{
	...
	if( IS_MCAST(pRA) ){
		...
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if(mac_addr_equal(pWapiSta->PeerMacAddr,pRA)){
    				bFindMatchPeer = true;
				break;
			}
		}
	...
	)
{
	...
	if (!list_empty(&(pWapiInfo->wapiSTAUsedList))){
		list_for_each_entry(pWapiSta, &pWapiInfo->wapiSTAUsedList, list) {
			if (mac_addr_equal(pWapiSta->PeerMacAddr, pTA)){
				bFindMatchPeer = true;
				break;
			}
		}
