...
_func_enter_;
	...
	if (!Adapter->mppriv.workparam.bcompleted)
		return NDIS_STATUS_NOT_ACCEPTED;
...
_func_enter_;
	...
	if (poid_par_priv->type_of_oid != QUERY_OID)
		return NDIS_STATUS_NOT_ACCEPTED;
			...
			if(pmp_wi_cntx->bmp_wi_progress){
...
NDIS_STATUS oid_rt_pro_encryption_ctrl_hdl(struct oid_par_priv *poid_par_priv)
{
	...
	*poid_par_priv->bytes_needed = sizeof(u8);
	...
	if (poid_par_priv->type_of_oid == SET_OID)
	{
		...
		#if 0
		if (!Adapter->registrypriv.software_encrypt) {
			if (!Adapter->registrypriv.software_decrypt)
		...
		if (!psecuritypriv->sw_encrypt && !psecuritypriv->sw_decrypt)
			encry_mode = HW_CONTROL;
		else if (!psecuritypriv->sw_encrypt && psecuritypriv->sw_decrypt)
			encry_mode = HW_ENCRY_SW_DECRY;
		else if (psecuritypriv->sw_encrypt && !psecuritypriv->sw_decrypt)
			encry_mode = SW_ENCRY_HW_DECRY;
		else if (psecuritypriv->sw_encrypt && psecuritypriv->sw_decrypt)
...
_func_enter_;
	...
	max_available_size = EFUSE_GetEfuseDefinition(Adapter, EFUSE_WIFI, TYPE_AVAILABLE_EFUSE_BYTES_TOTAL, _FALSE);
...
//	RT_TRACE(_module_mp_, _drv_info_, ("+oid_rt_pro_rw_efuse_pgpkt_hdl\n"));
	...
	*poid_par_priv->bytes_rw = 0;
	...
	if (poid_par_priv->type_of_oid == QUERY_OID)
	{
		...
		if (Efuse_PgPacketRead(Adapter, ppgpkt->offset, ppgpkt->data, _FALSE))
			*poid_par_priv->bytes_rw = poid_par_priv->information_buf_len;
		else
			status = NDIS_STATUS_FAILURE;
		...
		if (Efuse_PgPacketWrite(Adapter, ppgpkt->offset, ppgpkt->word_en, ppgpkt->data, _FALSE))
			*poid_par_priv->bytes_rw = poid_par_priv->information_buf_len;
		else
			status = NDIS_STATUS_FAILURE;
...
_func_enter_;
	...
	mapLen = EFUSE_GetEfuseDefinition(Adapter, EFUSE_WIFI, TYPE_EFUSE_MAP_LEN, _FALSE);
...
unsigned int mp_ioctl_xmit_packet_hdl(struct oid_par_priv *poid_par_priv)
{
	...
	if (poid_par_priv->type_of_oid == QUERY_OID) {
		...
		if (pparm->enable == 0) {
			...
			copy_mac_addr(pattrib->dst, pparm->da);
...
//	DBG_871X("+mp_ioctl_xmit_packet_hdl\n");
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, pethhdr->h_dest);
	copy_mac_addr(pwlanhdr->addr2, pethhdr->h_source);

	copy_mac_addr(pwlanhdr->addr3, addr3);
