...
static u32 go_add_group_info_attr(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if(psta->is_p2p_device)
		{
			...
			//P2P device address
			copy_mac_addr(pcur, psta->dev_addr);
			pcur += ETH_ALEN;

			//P2P interface address
			copy_mac_addr(pcur, psta->hwaddr);
			pcur += ETH_ALEN;

			*pcur = psta->dev_cap;
			pcur++;

			RTW_WB16(pcur, psta->config_methods);
			pcur += 2;
			...
			*pcur = psta->num_of_secdev_type;
			...
			{
				RTW_WB16(pcur, WPS_ATTR_DEVICE_NAME);
				pcur += 2;

				RTW_WB16(pcur, psta->dev_name_len);
				pcur += 2;
...
static void issue_group_disc_req(struct wifidirect_info *pwdinfo, u8 *da)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr2, pwdinfo->interface_addr);
	copy_mac_addr(pwlanhdr->addr3, pwdinfo->interface_addr);
...
static void issue_p2p_devdisc_resp(struct wifidirect_info *pwdinfo, u8 *da, u8 status, u8 dialogToken)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, da);
	copy_mac_addr(pwlanhdr->addr2, pwdinfo->device_addr);
	copy_mac_addr(pwlanhdr->addr3, pwdinfo->device_addr);
...
static void issue_p2p_provision_resp(struct wifidirect_info *pwdinfo, u8* raddr, u8* frame_body, u16 config_method)
{
	...
	*(u16*) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
	...
	//	Config Method
	//	Type:
	RTW_WB16(wpsie + wpsielen, WPS_ATTR_CONF_METHOD);
	wpsielen += 2;

	//	Length:
	RTW_WB16(wpsie + wpsielen, 0x0002);
	wpsielen += 2;

	//	Value:
	RTW_WB16(wpsie + wpsielen, config_method);
	wpsielen += 2;
...
u32 build_beacon_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	RTW_WB16(wfdie + wfdielen, 0x0006);
	wfdielen += 2;
	...
	if ( P2P_ROLE_GO == pwdinfo->role )
	{
		if ( is_any_client_associated( pwdinfo->padapter ) )
		{
			...
			RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_WSD );
		}
		else
		{
			//	WFD primary sink + available for WFD session + WiFi Direct mode + WSD (WFD Service Discovery)
			RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_SESSION_AVAIL | WFD_DEVINFO_WSD );
		}
	}
	else
	{
	...
	RTW_WB16(wfdie + wfdielen, pwfd_info->rtsp_ctrlport );
	wfdielen += 2;

	//	Value3:
	//	WFD Device Maximum Throughput
	//	300Mbps is the maximum throughput
	RTW_WB16(wfdie + wfdielen, 300);
	wfdielen += 2;
	...
	{
		copy_mac_addr( wfdie + wfdielen, &pmlmepriv->assoc_bssid[ 0 ]);
	}
	else
	{
		set_zero_mac_addr( wfdie + wfdielen );
	}
	...
	//	Length:
	//	Note: In the WFD specification, the size of length field is 2.
	RTW_WB16(wfdie + wfdielen, 0x0007);
	wfdielen += 2;
...
u32 build_probe_req_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	if ( 1 == pwdinfo->wfd_tdls_enable )
	{
		...
		RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type |
...
u32 build_probe_resp_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf, u8 tunneled)
{
		...
		if ( P2P_ROLE_GO == pwdinfo->role )
		{
			if ( is_any_client_associated( pwdinfo->padapter ) )
			{
				if ( pwdinfo->wfd_tdls_enable )
				{
					...
					RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_PC_TDLS | WFD_DEVINFO_HDCP_SUPPORT);
				}
				else
				{
					//	WiFi Direct mode + WSD ( WFD Service Discovery )
					RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_HDCP_SUPPORT);
				}
			}
			else
			{
				if ( pwdinfo->wfd_tdls_enable )
				{
					...
					RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_SESSION_AVAIL | WFD_DEVINFO_WSD | WFD_DEVINFO_PC_TDLS | WFD_DEVINFO_HDCP_SUPPORT);
				}
				else
				{
					//	available for WFD session + WiFi Direct mode + WSD ( WFD Service Discovery )
					RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_SESSION_AVAIL | WFD_DEVINFO_WSD | WFD_DEVINFO_HDCP_SUPPORT);
				}
			}
		}
		else
	...
	if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
	{
		...
		//	Length:
		//	Note: In the WFD specification, the size of length field is 2.
		RTW_WB16(wfdie + wfdielen, 0x0000);
		wfdielen += 2;
	...
	if ( ( tunneled == 0 ) && ( padapter->pbuddy_adapter->wdinfo.wfd_tdls_enable == 1 ) )
	{
		...
		//	Length:
		//	Note: In the WFD specification, the size of length field is 2.
		RTW_WB16(wfdie + wfdielen,  ETH_ALEN );
		wfdielen += 2;

		//	Value:
		//	Alternative MAC Address
		copy_mac_addr( wfdie + wfdielen, &padapter->pbuddy_adapter->eeprompriv.mac_addr[ 0 ]);
...
u32 build_nego_req_wfd_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_WSD | WFD_DEVINFO_SESSION_AVAIL);
	wfdielen += 2;
...
u32 build_probe_resp_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pbuf)
{
	...
	if( memchr_inv( pmlmepriv->sa_ext, 0, L2SDTA_SERVICE_VE_LEN ) )
	{
		widi_version = 35;
	}
	...
	RTW_WL16(p2pie + p2pielen, 0x0002);
	p2pielen += 2;
	...
	//	Length:
	RTW_WL16(p2pie + p2pielen, 0x0004);
	p2pielen += 2;

	//	Value:
	//	Availability Period
	RTW_WL16(p2pie + p2pielen, 0xFFFF);
	p2pielen += 2;
	...
	if( widi_version == 35 )
	{
		RTW_WL16(p2pie + p2pielen, 21 + 8 + pwdinfo->device_name_len);
	}
	else if( widi_version == 40 )
	{
		RTW_WL16(p2pie + p2pielen, 21 + 8 * pmlmepriv->num_p2p_sdt + pwdinfo->device_name_len);
	}
	else
#endif //CONFIG_INTEL_WIDI
	RTW_WL16(p2pie + p2pielen, 21 + pwdinfo->device_name_len);
	p2pielen += 2;
	...
	RTW_WB16(p2pie + p2pielen, pwdinfo->supported_wps_cm);
	p2pielen += 2;
	...
	if( widi_version == 40 )
	{
		//	Primary Device Type
		//	Category ID
		RTW_WB16(p2pie + p2pielen, pmlmepriv->p2p_pdt_cid );
		p2pielen += 2;

		//	OUI
		RTW_WN32(p2pie + p2pielen, WPS_OUI);
		p2pielen += 4;

		//	Sub Category ID
		RTW_WB16(p2pie + p2pielen, pmlmepriv->p2p_pdt_scid);
		p2pielen += 2;
	}
	else
#endif //CONFIG_INTEL_WIDI
	{
		//	Primary Device Type
		//	Category ID
		RTW_WB16(p2pie + p2pielen, WPS_PDT_CID_MULIT_MEDIA);
		p2pielen += 2;
		...
		//	Sub Category ID
		RTW_WB16(p2pie + p2pielen, WPS_PDT_SCID_MEDIA_SERVER);
		p2pielen += 2;
	}
	...
	if( widi_version == 35 )
	{
		...
		RTW_WB16(p2pie + p2pielen, WPS_PDT_CID_DISPLAYS);
		p2pielen += 2;

		RTW_WB32(p2pie + p2pielen, INTEL_DEV_TYPE_OUI);
		p2pielen += 4;

		RTW_WB16(p2pie + p2pielen, P2P_SCID_WIDI_CONSUMER_SINK);
		p2pielen += 2;
	}
	else if( widi_version == 40 )
	{
		...
		{
			RTW_WB16(p2pie + p2pielen, pmlmepriv->p2p_sdt_cid[i]);
			p2pielen += 2;

			RTW_WB32(p2pie + p2pielen, INTEL_DEV_TYPE_OUI);
			p2pielen += 4;

			RTW_WB16(p2pie + p2pielen, pmlmepriv->p2p_sdt_scid[i]);
			p2pielen += 2;
		}
	}
	else
	...
	//	Device Name
	//	Type:
	RTW_WB16(p2pie + p2pielen, WPS_ATTR_DEVICE_NAME);
	p2pielen += 2;

	//	Length:
	RTW_WB16(p2pie + p2pielen, pwdinfo->device_name_len);
	p2pielen += 2;
...
u32 build_prov_disc_request_p2p_ie(struct wifidirect_info *pwdinfo, u8 *pbuf, u8* pssid, u8 ussidlen, u8* pdev_raddr )
{
	...
	{
		RTW_WB16(p2pie + p2pielen, WPS_CONFIG_METHOD_PBC);
	}
	else
	{
		RTW_WB16(p2pie + p2pielen, WPS_CONFIG_METHOD_DISPLAY);
	}
	...
	if ( rtw_p2p_chk_role(pwdinfo, P2P_ROLE_CLIENT) )
	{
		...
		//	Length:
		RTW_WL16(p2pie + p2pielen, ETH_ALEN + ussidlen);
		p2pielen += 2;

		//	Value:
		copy_mac_addr( p2pie + p2pielen, pdev_raddr);
		p2pielen += ETH_ALEN;
	...
	while ( p2p_ie )
	{
		...
		if(rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL, (uint*)&attr_contentlen))
		{
			...
			if(pattr_content)
			{
				...
				copy_mac_addr(psta->dev_addr, pattr_content);//P2P Device Address
...
u32 process_p2p_devdisc_req(struct wifidirect_info *pwdinfo, u8 *pframe, uint len)
{
	...
	if ( (p2p_ie=rtw_get_p2p_ie( frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)) )
	{
		...
		if(rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid, &attr_contentlen))
		{
			...
			if (mac_addr_equal(pwdinfo->device_addr, groupid) &&
				...
				if(rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_ID, dev_addr, &attr_contentlen))
				{
					...
					while (!(rtw_end_of_queue_search(phead, plist)) )
					{
						...
						if (psta->is_p2p_device && (psta->dev_cap & P2P_DEVCAP_CLIENT_DISCOVERABILITY) &&
							mac_addr_equal(psta->dev_addr, dev_addr))
						{
	...
	while ( p2p_ie )
	{
		...
		if ( rtw_get_p2p_attr_content(p2p_ie, p2p_ielen, P2P_ATTR_GO_INTENT , &attr_content, &attr_contentlen) )
		{
			...
			if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
			{
				...
				copy_mac_addr( pwdinfo->groupid_info.go_device_addr, pwdinfo->device_addr);
		...
		if ( rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_INTENTED_IF_ADDR, pwdinfo->p2p_peer_interface_addr, &attr_contentlen ) )
		{
			...
			{
				set_zero_mac_addr( pwdinfo->p2p_peer_interface_addr );
			}
		}
...
u8 process_p2p_group_negotation_resp( struct wifidirect_info *pwdinfo, u8 *pframe, uint len )
{
	...
	else
	{
		...
		while ( p2p_ie )	//	Found the P2P IE.
		{
			...
			if ( rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, groupid, &attr_contentlen) )
			{
				copy_mac_addr( pwdinfo->groupid_info.go_device_addr, &groupid[0]);
...
void rtw_append_wfd_ie(_adapter *padapter, u8 *buf, u32* len)
{
	...
	{
		action = frame_body[1];
		if (action == ACT_PUBLIC_VENDOR && RTW_RN32(frame_body+2) == P2P_OUI)
		{
			...
			switch( OUI_Subtype )//OUI Subtype
			{
				...
				case P2P_GO_NEGO_REQ:
				{
					...
					if (tx) {
						...
						if(!pwdev_priv->provdisc_req_issued )
					...
					copy_mac_addr(nego_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf));
					...
					if (!tx) {
						...
						if((check_buddy_fwstate(padapter, _FW_LINKED)
							|| check_buddy_fwstate(padapter, WIFI_AP_STATE))
							&& !rtw_chk_p2pie_ch_list_with_buddy(padapter, frame_body, len-sizeof(struct rtw_ieee80211_hdr_3addr)))
						{
				...
				case P2P_GO_NEGO_RESP:
				{
					...
					if (nego_info->token == dialogToken && nego_info->state == 0
						&& mac_addr_equal(nego_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf))
					) {
				...
				case P2P_INVIT_REQ:
				{
					...
					copy_mac_addr(invit_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf));
					...
					if (!tx) {
						...
						if(check_buddy_fwstate(padapter, _FW_LINKED)
							|| check_buddy_fwstate(padapter, WIFI_AP_STATE)) {
							if (op_ch != -1 && !rtw_chk_p2pie_op_ch_with_buddy(padapter, frame_body, len-sizeof(struct rtw_ieee80211_hdr_3addr))) {
								DBG_871X(FUNC_ADPT_FMT" op_ch:%u has no intersect with buddy\n", FUNC_ADPT_ARG(padapter), op_ch);
								rtw_change_p2pie_ch_list(padapter, frame_body, len-sizeof(struct rtw_ieee80211_hdr_3addr), 0);
							} else if (!rtw_chk_p2pie_ch_list_with_buddy(padapter, frame_body, len-sizeof(struct rtw_ieee80211_hdr_3addr))) {
								DBG_871X(FUNC_ADPT_FMT" ch_list has no intersect with buddy\n", FUNC_ADPT_ARG(padapter));
								rtw_change_p2pie_ch_list(padapter, frame_body, len-sizeof(struct rtw_ieee80211_hdr_3addr), 0);
							}
						}
				...
				case P2P_INVIT_RESP:
				{
					...
					if (invit_info->token == dialogToken && invit_info->state == 0
						&& mac_addr_equal(invit_info->peer_mac, tx ? GetAddr1Ptr(buf) : GetAddr2Ptr(buf))
					) {
...
void rtw_init_wifidirect_addrs(_adapter* padapter, u8 *dev_addr, u8 *iface_addr)
{
	...
	if (dev_addr) {
		copy_mac_addr(pwdinfo->device_addr, dev_addr);
	}
	if (iface_addr) {
		copy_mac_addr(pwdinfo->interface_addr, iface_addr);
	}
#endif
}
...
// For WiDi purpose: Use CFG80211 interface but controled WFD/RDS frame by driver itself.
	...
	pwdinfo->wfd_tdls_enable = 0;
	set_zero_mac_addr( pwdinfo->p2p_peer_interface_addr );
	set_zero_mac_addr( pwdinfo->p2p_peer_device_addr );
