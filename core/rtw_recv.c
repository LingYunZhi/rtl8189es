...
sint OnTDLS(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	u8 category_field = 1;
#ifdef CONFIG_WFD
#define WFA_OUI_REQ		PACK_NE32(0x50, 0x6f, 0x9a, 0x04)
#define WFA_OUI_RESP	PACK_NE32(0x50, 0x6f, 0x9a, 0x05)
#endif //CONFIG_WFD
	...
	switch(*paction){
		...
		case 0x50:	//First byte of WFA OUI
			if (RTW_RN32(paction) == WFA_OUI_REQ)	//Probe request frame
			{
			...
			else if (RTW_RN32(paction) == WFA_OUI_RESP)	//Probe response frame
			{
					//WFDTDLS: for sigma test, do not setup direct link automatically
					ptdlsinfo->dev_discovered = 1;
...
void count_rx_stats(_adapter *padapter, union recv_frame *prframe, struct sta_info*sta)
{
	...
	if( (!is_broadcast_mac_addr(pattrib->dst)) && (!IS_MCAST(pattrib->dst))){
...
)
{
	...
	u8 * sta_addr = NULL;
	bool bmcast = IS_MCAST(pattrib->dst);
...
_func_enter_;
		...
		check_fwstate(pmlmepriv, WIFI_ADHOC_MASTER_STATE))
	{
		...
		if (mac_addr_equal(myhwaddr, pattrib->src)){
			RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,(" SA==myself \n"));
			ret= _FAIL;
			goto exit;
		}

		if(!mac_addr_equal(myhwaddr, pattrib->dst) && (!bmcast) ){
			ret= _FAIL;
			goto exit;
		}

		if(is_zero_mac_addr(pattrib->bssid) ||
		   is_zero_mac_addr(mybssid) ||
		   !mac_addr_equal(pattrib->bssid, mybssid) ) {
			ret= _FAIL;
			goto exit;
		}
	...
	else if(check_fwstate(pmlmepriv, WIFI_STATION_STATE))
	{
		...
		if(ptdlsinfo->setup_state == TDLS_LINKED_STATE){
			...
			else if(ptdls_sta->tdls_sta_state&TDLS_LINKED_STATE)
			{
				...
				if (RTW_RN16(psnap_type) == SNAP_ETH_TYPE_TDLS) {
...
#endif //CONFIG_TDLS
		{
			...
			if(!mac_addr_equal(pattrib->bssid, pattrib->src) )
			{
	...
	else if(check_fwstate(pmlmepriv, WIFI_AP_STATE))
	{
		...
		else // not mc-frame
		{
			...
			if(!mac_addr_equal(pattrib->bssid, pattrib->dst)) {
				ret= _FAIL;
				goto exit;
			}
	...
	{
		copy_mac_addr(pattrib->dst, GetAddr1Ptr(ptr));
		copy_mac_addr(pattrib->src, GetAddr2Ptr(ptr));
		copy_mac_addr(pattrib->bssid, GetAddr3Ptr(ptr));
...
_func_enter_;
		     ...
		     check_fwstate(pmlmepriv, _FW_LINKED))
	{
		...
		//
		copy_mac_addr(pattrib->bssid,  mybssid);
...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE))
	{
		...
		if(!mac_addr_equal(pattrib->bssid, mybssid))
		{
			ret= _FAIL;
			goto exit;
		}
		...
		*psta = rtw_get_stainfo(pstapriv, pattrib->src);
		...
		u8 *myhwaddr = myid(&adapter->eeprompriv);
		if (!mac_addr_equal(pattrib->ra, myhwaddr)) {
			ret = RTW_RX_HANDLED;
			goto exit;
		}
		DBG_871X("issue_deauth to sta=" MAC_FMT " for the reason(7)\n", MAC_ARG(pattrib->src));
		...
		if((psta->state&WIFI_SLEEP_STATE) && (pstapriv->sta_dz_bitmap&BIT(psta->aid)))
		{
			...
			if (!rtw_end_of_queue_search(xmitframe_phead, xmitframe_plist))
			{
				...
				if(psta->sleepq_len==0)
				{
					...
					update_beacon(padapter, _TIM_IE_, 0, _FALSE);
				}
...
sint validate_recv_mgnt_frame(PADAPTER padapter, union recv_frame *precv_frame)
{
...

	{
		...
		if (psta) {
			...
			if (GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
				...
				if (mac_addr_equal(padapter->eeprompriv.mac_addr, GetAddr1Ptr(precv_frame->u.hdr.rx_data)))
	...
	if(padapter->proximity.proxim_on)
	{
		...
		copy_mac_addr(pattrib->dst, pda);
		copy_mac_addr(pattrib->src, psa);

		copy_mac_addr(pattrib->bssid, pbssid);

	switch(pattrib->to_fr_ds)
	{
		case 0:
			copy_mac_addr(pattrib->ra, pda);
			copy_mac_addr(pattrib->ta, psa);
			break;

		case 1:
			copy_mac_addr(pattrib->ra, pda);
			copy_mac_addr(pattrib->ta, pbssid);
			break;

		case 2:
			copy_mac_addr(pattrib->ra, pbssid);
			copy_mac_addr(pattrib->ta, psa);
			break;

		case 3:
			copy_mac_addr(pattrib->ra, GetAddr1Ptr(ptr));
			copy_mac_addr(pattrib->ta, GetAddr2Ptr(ptr));
...
_func_enter_;
	...
	if((!memcmp(psnap, rtw_rfc1042_header, SNAP_SIZE) &&
		RTW_RN16(psnap_type) != SNAP_ETH_TYPE_IPX &&
		RTW_RN16(psnap_type) != SNAP_ETH_TYPE_APPLETALK_AARP)||
	...
	eth_type= RTW_RB16(ptr + rmv_len); //pattrib->ether_type
	...
	copy_mac_addr(ptr, pattrib->dst);
	copy_mac_addr(ptr+ETH_ALEN, pattrib->src);
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while(a_len > ETH_HLEN) {
		...
		a_len -= nSubframe_Length;
		if(a_len != 0 && ((nSubframe_Length + ETH_HLEN) & (4-1)) != 0) {
	...
	for(i=0; i<nr_subframes; i++){
		sub_skb = subframes[i];
		/* convert hdr + possible LLC headers into Ethernet header */
		eth_type = RTW_RB16(&sub_skb->data[6]);
		if (sub_skb->len >= 8 &&
			...
			copy_mac_addr(skb_push(sub_skb, ETH_ALEN), pattrib->src);
			copy_mac_addr(skb_push(sub_skb, ETH_ALEN), pattrib->dst);
		} else {
			/* Leave Ethernet header part of hdr and full payload */
			RTW_WB16(skb_push(sub_skb, 2), sub_skb->len);
			copy_mac_addr(skb_push(sub_skb, ETH_ALEN), pattrib->src);
			copy_mac_addr(skb_push(sub_skb, ETH_ALEN), pattrib->dst);
		}
...
int check_indicate_seq(struct recv_reorder_ctrl *preorder_ctrl, u16 seq_num)
{
	...
	else if(SN_LESS(wend, seq_num))
	{
		...
		preorder_ctrl->indicate_seq = (seq_num - (wsize - 1)) & 0xFFF;
...
void rtw_reordering_ctrl_timeout_handler(void *pcontext)
{
	...
	if(recv_indicatepkts_in_order(padapter, preorder_ctrl, _TRUE))
	{
		_set_timer(&preorder_ctrl->reordering_ctrl_timer, REORDER_WAIT_TIME);
	}
...
void rtw_signal_stat_timer_hdl(RTW_TIMER_HDL_ARGS)
{
	...
	if(adapter->recvpriv.is_signal_dbg) {
		...
		#ifdef CONFIG_CONCURRENT_MODE
		if (check_buddy_fwstate(adapter, _FW_UNDER_SURVEY))
			goto set_timer;
		#endif
		...
		if (tmp_s >= 100 * _alpha)
			...
			tmp_s = (tmp_s + _alpha - 1) / _alpha;

		tmp_q = (avg_signal_qual+(_alpha-1)*recvpriv->signal_qual);
		if (tmp_q >= 100 * _alpha)
			tmp_q = 100;
		else
			tmp_q = (tmp_q + _alpha - 1) / _alpha;
