...
#ifdef CONFIG_TDLS
extern const unsigned char MCS_rate_2R[16];
extern const unsigned char MCS_rate_1R[16];
...
//	SetToDs(fctrl);
	...
	copy_mac_addr(pwlanhdr->addr1, ptdls_sta->hwaddr);
...
void rtw_tdls_process_ht_cap(_adapter *adapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	{
		if(adapter->registrypriv.ht_enable)
		{
...
void rtw_tdls_process_wfd_ie(struct tdls_info *ptdlsinfo, u8 *ptr, u8 length)
{
	...
	while( wfd_offset )
	{
		...
		if ( attr_contentlen )
		{
			ptdlsinfo->wfd_info->peer_rtsp_ctrlport = RTW_RB16( attr_content + 2 );
...
void issue_tunneled_probe_req(_adapter *padapter)
{
	...
	copy_mac_addr(pattrib->src, myid(&(padapter->eeprompriv)));

	copy_mac_addr(pattrib->ra, get_bssid(pmlmepriv));
...
void issue_tunneled_probe_rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	copy_mac_addr(pattrib->dst, rx_pkt_pattrib->src);
...
void issue_tdls_setup_req(_adapter *padapter, u8 *mac_addr)
{
	...
	copy_mac_addr(pattrib->dst, mac_addr);
	copy_mac_addr(pattrib->src, myid(&(padapter->eeprompriv)));
...
void issue_tdls_setup_rsp(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	copy_mac_addr(pattrib->ra, rx_pkt_pattrib->bssid);
	copy_mac_addr(pattrib->ta, pattrib->src);
...
void issue_tdls_dis_rsp(_adapter *padapter, union recv_frame *precv_frame, u8 dialog)
{
	...
	*(fctrl) = 0;
	...
	copy_mac_addr(pwlanhdr->addr1, rx_pkt_pattrib->src);
	copy_mac_addr(pattrib->dst, pwlanhdr->addr1);

	copy_mac_addr(pwlanhdr->addr2, myid(&(padapter->eeprompriv)));
	copy_mac_addr(pattrib->src, pwlanhdr->addr2);

	copy_mac_addr(pwlanhdr->addr3, rx_pkt_pattrib->bssid);
	copy_mac_addr(pattrib->ra, pwlanhdr->addr3);
...
void issue_tdls_peer_traffic_indication(_adapter *padapter, struct sta_info *ptdls_sta)
{
	...
	copy_mac_addr(pattrib->dst, ptdls_sta->hwaddr);
	copy_mac_addr(pattrib->src, myid(&(padapter->eeprompriv)));
...
/*
	...
	if(xmitframe_enqueue_for_tdls_sleeping_sta(padapter, pmgntframe)){
...
sint On_TDLS_Dis_Rsp(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if(ptdls_sta != NULL)
	{
		...
		if( ptdlsinfo->sta_maximum && (ptdls_sta->alive_count >= 1) )
		{
			if(is_zero_mac_addr(ptdlsinfo->ss_record.macaddr))
			{
				copy_mac_addr(ptdlsinfo->ss_record.macaddr, psa);
	...
	for(j=FIXED_IE; j<parsing_length;){
		...
		switch (pIE->ElementID)
		{
			case _LINK_ID_IE_:
				...
				dst = pIE->data + 12;
				if(!is_broadcast_mac_addr(dst) && !mac_addr_equal(myid(&(adapter->eeprompriv)), dst))
				{
					goto exit;
				}
				break;
	...
	if(ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE){
		if(wmmps_ac && state)
		{
			...
			while (!(rtw_end_of_queue_search(xmitframe_phead, xmitframe_plist)) )
			{
				...
				if(adapter->HalFunc->hal_xmit(adapter, pxmitframe))
				{
...
void wfd_ie_tdls(_adapter * padapter, u8 *pframe, u32 *pktlen )
{
	...
	RTW_WB16(wfdie + wfdielen, pwfd_info->wfd_device_type | WFD_DEVINFO_SESSION_AVAIL
	...
	//	Length:
	//	Note: In the WFD specification, the size of length field is 2.
	RTW_WB16(wfdie + wfdielen, 0x0005);
	wfdielen += 2;
...
void rtw_build_tdls_setup_req_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	//Link identifier
	copy_mac_addr(link_id_addr, pattrib->ra);
	copy_mac_addr((link_id_addr+6), pattrib->src);
	copy_mac_addr((link_id_addr+12), pattrib->dst);
...
void rtw_build_tdls_setup_rsp_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, category, &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, action, &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 2, ptdls_sta->stat_code, &(pattrib->pktlen));
	...
	plinkid_ie = pframe;
	copy_mac_addr(link_id_addr, pattrib->ra);
	copy_mac_addr((link_id_addr+6), pattrib->dst);
	copy_mac_addr((link_id_addr+12), pattrib->src);
	pframe = rtw_set_ie(pframe, _LINK_ID_IE_,  18, link_id_addr, &(pattrib->pktlen));
...
void rtw_build_tdls_teardown_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, category, &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, action, &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 1, ptdls_sta->stat_code, &(pattrib->pktlen));
...
void rtw_build_tunneled_probe_req_ies(_adapter * padapter, struct xmit_frame * pxmitframe, u8 *pframe)
{
	...
	pframe = rtw_set_fixed_ie(pframe, 1, category, &(pattrib->pktlen));
	pframe = rtw_set_fixed_ie(pframe, 3, PACK_NE24(0x50, 0x6f, 0x9a), &(pattrib->pktlen));
