...
const u8 zero_mac_addr[ETH_ALEN] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
const u8 broadcast_mac_addr[ETH_ALEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

//-----------------------------------------------------------
// for adhoc-master to generate ie and provide supported-rate to fw
//-----------------------------------------------------------

static const u8 	WIFI_CCKRATES[] =
{(IEEE80211_CCK_RATE_1MB | IEEE80211_BASIC_RATE_MASK),
 (IEEE80211_CCK_RATE_2MB | IEEE80211_BASIC_RATE_MASK),
 (IEEE80211_CCK_RATE_5MB | IEEE80211_BASIC_RATE_MASK),
 (IEEE80211_CCK_RATE_11MB | IEEE80211_BASIC_RATE_MASK)};

static const u8 	WIFI_OFDMRATES[] =
...
{
	static const u8 dot11_rate_table[]={2,4,11,22,12,18,24,36,48,72,96,108,0}; // last element must be zero!!
	...
	u8 flags, u16 reason, u16 precedence)
{
	...
	ie_data[0] = ttl;
	ie_data[1] = flags;
	RTW_WL16((u8*)&ie_data[2], reason);
	RTW_WL16((u8*)&ie_data[4], precedence);
 ...
 */
u8 *rtw_get_ie_ex(u8 *in_ie, uint in_len, u8 eid, u32 oui, u8 *ie, uint *ielen)
{
	uint cnt;
	u8 *target_ie = NULL;
	...
	for (cnt = 0; cnt<in_len; cnt+=in_ie[cnt+1]+2)
	{
		if (eid == in_ie[cnt] &&
			(!oui || RTW_RN32(&in_ie[cnt+2]) == oui))
		{
			target_ie = &in_ie[cnt];
 ...
 */
int rtw_ies_remove_ie(u8 *ies, uint *ies_len, uint offset, u8 eid, u32 oui)
{
	int ret = _FAIL;
	u8 *target_ie;
	u32 target_ielen;
	...
	while (1) {
		target_ie = rtw_get_ie_ex(start, search_len, eid, oui, NULL, &target_ielen);
		if (target_ie && target_ielen) {
			u8 *remain_ies = target_ie + target_ielen;
			uint remain_len = search_len - (remain_ies - start);

			memove(target_ie, remain_ies, remain_len);
...
unsigned char *rtw_get_wpa_ie(unsigned char *pie, int *wpa_ie_len, int limit)
{
	...
	while(1)
	{
		...
		if (pbuf) {
			...
			if (RTW_RN32(pbuf + 2) != WPA_OUI) {

				goto check_next_ie;
			}

			//check version...
			val16 = RTW_RL16(pbuf + 6);
			if (val16 != 0x0001)
				goto check_next_ie;
...
int rtw_parse_wpa2_ie(u8* rsn_ie, int rsn_ie_len, int *group_cipher, int *pairwise_cipher, int *is_8021x)
{
	...
	if (is_8021x) {
		if (left >= 6) {
			pos += 2;
			if (RTW_RN32(pos) == RSN_CIPHER_SUITE_WEP40) {
				RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("%s (): there has 802.1x auth\n", __FUNCTION__));
				*is_8021x = 1;
			}
		}
...
#define WAPI_OUI1	PACK_NE32(0x0,0x14,0x72,0x01)
#define WAPI_OUI2	PACK_NE32(0x0,0x14,0x72,0x02)
...
_func_enter_;
	...
	while(cnt<in_len)
	{
		...
		if (authmode==_WAPI_IE_ && (RTW_RN32(&in_ie[cnt+6]) == WAPI_OUI1 ||
				       	RTW_RN32(&in_ie[cnt+6]) == WAPI_OUI2))
		{
			if (wapi_ie) {
...
_func_enter_;
	...
	while(cnt<in_len)
	{
		...
		if (authmode == _WPA_IE_ID_ && RTW_RN32(&in_ie[cnt+2]) == WPA_OUI)
		{
...
u8 rtw_is_wps_ie(u8 *ie_ptr, uint *wps_ielen)
{
	...
	if (eid == _WPA_IE_ID_ && RTW_RN32(&ie_ptr[2]) == WPS_OUI)
	{
...
u8 *rtw_get_wps_ie(u8 *in_ie, uint in_len, u8 *wps_ie, uint *wps_ielen)
{
	...
	while(cnt<in_len)
	{
		...
		if(eid==_WPA_IE_ID_ && RTW_RN32(&in_ie[cnt+2]) == WPS_OUI)
		{
			wpsie_ptr = &in_ie[cnt];
...
u8 *rtw_get_wps_attr(u8 *wps_ie, uint wps_ielen, u16 target_attr_id ,u8 *buf_attr, u32 *len_attr)
{
	...
	while(attr_ptr - wps_ie < wps_ielen)
	{
		...
		u16 attr_id = RTW_RB16(attr_ptr);
		u16 attr_data_len = RTW_RB16(attr_ptr + 2);
...
{
    return (hex_to_bin(hch) * 10 ) + hex_to_bin(lch);
}
...
{
    return (hex_to_bin(hch) * 100) + (hex_to_bin(mch) * 10 ) + hex_to_bin(lch);
}
...
u32 rtw_get_p2p_merged_ies_len(u8 *in_ie, u32 in_len)
{
	...
	while( i < in_len)
	{
		...
		if( pIE->ElementID == _VENDOR_SPECIFIC_IE_ && RTW_RN32(pIE->data) == P2P_OUI )
		{
...
int rtw_p2p_merge_ies(u8 *in_ie, u32 in_len, u8 *merge_ie)
{
	PNDIS_802_11_VARIABLE_IEs	pIE;
	u8 len = 0;
	static const u8 ELOUI[6] = { 0xDD, 0x00, 0x50, 0x6f, 0x9a, 0x09 };	//EID;Len;OUI, Len would copy at the end of function
	int i=0;
...
u8 *rtw_get_p2p_ie(u8 *in_ie, int in_len, u8 *p2p_ie, uint *p2p_ielen)
{
	...
	for (cnt = 0; cnt<in_len; cnt += in_ie[ cnt + 1 ] +2)
	{
		...
		if (eid == _VENDOR_SPECIFIC_IE_ && RTW_RN32(&in_ie[cnt+2]) == P2P_OUI)
		{
...
u8 *rtw_get_p2p_attr(u8 *p2p_ie, uint p2p_ielen, u8 target_attr_id ,u8 *buf_attr, u32 *len_attr)
{
	...
	while(attr_ptr - p2p_ie < p2p_ielen)
	{
		...
		u8 attr_id = *attr_ptr;
		u16 attr_data_len = RTW_RL16(attr_ptr + 1);
...
int rtw_get_wfd_ie(u8 *in_ie, int in_len, u8 *wfd_ie, uint *wfd_ielen)
{
	...
	for (cnt = 0; cnt<in_len; cnt += in_ie[ cnt + 1 ] +2)
	{
		...
		if (eid == _VENDOR_SPECIFIC_IE_ && RTW_RN32(&in_ie[cnt+2]) == WFD_OUI)
		{
			if ( wfd_ie != NULL )
			{
...
int rtw_get_wfd_attr_content(u8 *wfd_ie, uint wfd_ielen, u8 target_attr_id ,u8 *attr_content, uint *attr_contentlen)
{
	...
	if (wfd_ie[ 0 ] != _VENDOR_SPECIFIC_IE_ ||
		RTW_RN32(wfd_ie + 2) != WFD_OUI)
	{
		return( match );
	}

	//	1 ( WFD IE ) + 1 ( Length ) + 3 ( OUI ) + 1 ( OUI Type )
	for (cnt = 6; cnt < wfd_ielen; cnt += attrlen + 3)
	{
		attrlen = RTW_RB16(wfd_ie + cnt + 1);
...
{
	static const u16 mcs_rates[2][2][2][8] = {
		{ // RF_1T1R
			{ // !bw_40MHz
				// !short_GI_20
	...
	if (rf_type != RF_1T1R && MCS_rate[1])
		return mcs_rates[1][!!bw_40MHz][!!(bw_40MHz ? short_GI_40 : short_GI_20)][__fls(MCS_rate[1])];

	if (!MCS_rate[0])
		return 0;
	return mcs_rates[0][!!bw_40MHz][!!(bw_40MHz ? short_GI_40 : short_GI_20)][__fls(MCS_rate[0])];
}
