...
static __inline__ unsigned char *__nat25_find_pppoe_tag(struct pppoe_hdr *ph, unsigned short type)
{
	...
	while((cur_ptr - start_ptr) < ntohs(ph->length)) {
		...
		tagType = RTW_RB16(&cur_ptr[0]);
		tagLen  = RTW_RB16(&cur_ptr[2]);
		if(tagType == type)
			return cur_ptr;
				...
				unsigned int *ipxNetAddr, unsigned char *ipxNodeAddr)
{
	...
	RTW_WN32(networkAddr + 1, *ipxNetAddr);
	copy_mac_addr(networkAddr + 5, ipxNodeAddr);
}
				...
				unsigned int *ipxNetAddr, unsigned short *ipxSocketAddr)
{
	...
	networkAddr[0] = NAT25_IPX;
	RTW_WN32(networkAddr + 1, *ipxNetAddr);
	RTW_WN16(networkAddr + 5, *ipxSocketAddr);
}
				...
				unsigned short *network, unsigned char *node)
{
	...
	RTW_WN16(networkAddr + 1, *network);
	networkAddr[3] = *node;
}
				...
				unsigned char *ac_mac, unsigned short *sid)
{
	...
	RTW_WN16(networkAddr + 1, *sid);
	copy_mac_addr(networkAddr + 3, ac_mac);
}
				...
				unsigned int *ipAddr)
{
	...
	memcpy(networkAddr + 1, ipAddr, 16);
}
...
{
	while (len > 0) {
		if (data[0] == tag && data[1] == len8b && len >= len8b * 8)
			return data + 2;
...
static int checkIPMcAndReplace(_adapter *priv, struct sk_buff *skb, unsigned int *dst_ip)
{
	...
	while (plist != phead) {
		...
		for (i=0; i<MAX_IP_MC_ENTRY; i++) {
			if (pstat->ipmc[i].used && RTW_RN24(&pstat->ipmc[i].mcmac[3]) == RTW_RN24(((u8*)dst_ip) + 1)) {
				copy_mac_addr(skb->data, pstat->ipmc[i].mcmac);
				return 1;
			}
		}
	}
