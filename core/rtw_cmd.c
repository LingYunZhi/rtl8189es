...
{
	if(adapter->cmdThread && adapter->cmdpriv.cmdthd_running
...
_func_enter_;
	...
	*/
	...
	{
		copy_mac_addr( &pmlmepriv->assoc_bssid[ 0 ], &pnetwork->network.MacAddress[ 0 ]);
	}
...
_func_enter_;
	...
	copy_mac_addr(psetstakey_para->addr, sta->hwaddr);
...
_func_enter_;
	...
	copy_mac_addr(psetassocsta_para->addr, mac_addr);
...
_func_enter_;
	...
	copy_mac_addr(paddbareq_parm->addr, addr);
...
_func_enter_;
	...
	copy_mac_addr(TDLSoption->addr, addr);
...
void dynamic_chk_wk_hdl(_adapter *padapter, u8 *pbuf, int sz)
{
	...
	//if(!check_fwstate(pmlmepriv, _FW_UNDER_LINKING|_FW_UNDER_SURVEY))
	{
