...
#define ARTHEROS_OUI1	PACK_NE24(0x00, 0x03, 0x7f)
#define ARTHEROS_OUI2	PACK_NE24(0x00, 0x13, 0x74)

#define BROADCOM_OUI1	PACK_NE24(0x00, 0x10, 0x18)
#define BROADCOM_OUI2	PACK_NE24(0x00, 0x0a, 0xf7)
#define BROADCOM_OUI3	PACK_NE24(0x00, 0x05, 0xb5)

#define CISCO_OUI		PACK_NE24(0x00, 0x40, 0x96)
#define MARVELL_OUI		PACK_NE24(0x00, 0x50, 0x43)
#define RALINK_OUI		PACK_NE24(0x00, 0x0c, 0x43)
#define REALTEK_OUI		PACK_NE24(0x00, 0xe0, 0x4c)
#define AIRGOCAP_OUI	PACK_NE24(0x00, 0x0a, 0xf5)
#define EPIGRAM_OUI		PACK_NE24(0x00, 0x90, 0x4c)

extern const unsigned char	MCS_rate_2R[16];
#ifdef CONFIG_DISABLE_MCS13TO15
extern const unsigned char	MCS_rate_2R_MCS13TO15_OFF[16];
...
int rtw_check_bcn_info(ADAPTER *Adapter, u8 *pframe, u32 packet_len)
{
	...
	if (!mac_addr_equal(cur_network->network.MacAddress, pbssid)) {
...
unsigned int is_ap_in_tkip(_adapter *padapter)
{
	...
	if (rtw_get_capability((WLAN_BSSID_EX *)cur_network) & WLAN_CAPABILITY_PRIVACY)
	{
		for (i = sizeof(NDIS_802_11_FIXED_IEs); i < pmlmeinfo->network.IELength;)
		{
			...
			{
				case _VENDOR_SPECIFIC_IE_:
					if (RTW_RN32(pIE->data) == WPA_OUI && RTW_RN32(pIE->data + 12) == WPA_TKIP_CIPHER)
					{
						return _TRUE;
					}
					break;
				...
				case _RSN_IE_2_:
					if (RTW_RN32(pIE->data + 8) == RSN_TKIP_CIPHER)
					{
						return _TRUE;
					}
			...
			switch (pIE->ElementID)
			{
				case _VENDOR_SPECIFIC_IE_:
					if (RTW_RN32(pIE->data) == WPA_OUI &&
						(RTW_RN32(pIE->data + 12) == WPA_CIPHER_SUITE_CCMP ||
						 RTW_RN32(pIE->data + 16) == WPA_CIPHER_SUITE_CCMP))
						return _FALSE;
					break;

				case _RSN_IE_2_:
					if  (RTW_RN32(pIE->data + 8) == RSN_CIPHER_SUITE_CCMP ||
					     RTW_RN32(pIE->data + 12) == RSN_CIPHER_SUITE_CCMP)
					return _FALSE;
...
{
	int mcs_rate = RTW_RL16(pHT_caps->u.HT_cap_element.MCS_rate);
	if (mcs_rate)
		return __fls(mcs_rate);
	return -1;
}
