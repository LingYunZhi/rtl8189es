...
void	expire_timeout_chk(_adapter *padapter)
{
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if (mac_addr_equal(pstapriv->atmel_rc_pattern, psta->hwaddr))
			continue;
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
	...
	copy_mac_addr(pbss_network->MacAddress, myid(&(padapter->eeprompriv)));

	//beacon interval
	p = rtw_get_beacon_interval_from_ie(ie);//ie + 8;	// 8: TimeStamp, 2: Beacon Interval 2:Capability
	//pbss_network->Configuration.BeaconPeriod = le16_to_cpu(*(unsigned short*)p);
	pbss_network->Configuration.BeaconPeriod = RTW_RL16(p);
...
int rtw_acl_add_sta(_adapter *padapter, u8 *addr)
{
	...
	while (!rtw_end_of_queue_search(phead, plist))
	{
		...
		if (mac_addr_equal(paclnode->addr, addr))
		{
			if (paclnode->valid)
			{
				added = _TRUE;
	...
	for(i=0; i< NUM_ACL; i++)
	{
		...
		if(!paclnode->valid)
		{
			...
			copy_mac_addr(paclnode->addr, addr);
...
static void update_bcn_vendor_spec_ie(_adapter *padapter, u32 oui)
{
	DBG_871X("%s\n", __FUNCTION__);
...
void update_beacon(_adapter *padapter, u8 ie_id, u32 oui, u8 tx)
{
	_irqL irqL;
...
void bss_cap_update_on_sta_join(_adapter *padapter, struct sta_info *psta)
{
	...
	if(!(psta->flags & WLAN_STA_SHORT_PREAMBLE))
	{
		if(!psta->no_short_preamble_set)
		{
		     		...
		     		(pmlmepriv->num_sta_no_short_preamble == 1))
			{
				...
				update_beacon(padapter, 0xFF, 0, _TRUE);
			}
	...
	if(psta->flags & WLAN_STA_NONERP)
	{
		if(!psta->nonerp_set)
		{
			...
			{
				beacon_updated = _TRUE;
				update_beacon(padapter, _ERPINFO_IE_, 0, _TRUE);
			}
		}
	...
	{
		update_beacon(padapter, _HT_CAPABILITY_IE_, 0, _FALSE);
		update_beacon(padapter, _HT_ADD_INFO_IE_, 0, _TRUE);
	}
...
int rtw_ap_inform_ch_switch(_adapter *padapter, u8 new_ch, u8 ch_offset)
{
	...
	issue_action_spct_ch_switch(padapter, broadcast_mac_addr, new_ch, ch_offset);
...
int rtw_sta_flush(_adapter *padapter)
{
	...
	issue_deauth(padapter, broadcast_mac_addr, WLAN_REASON_DEAUTH_LEAVING);
