...
#define P802_1H_OUI	PACK_NE24(0x00, 0x00, 0xf8)
#define RFC1042_OUI	PACK_NE24(0x00, 0x00, 0x00)
...
static s32 update_attrib(_adapter *padapter, _pkt *pkt, struct pkt_attrib *pattrib)
{
 ...
 _func_enter_;
	...
	copy_mac_addr(pattrib->dst, &etherhdr.h_dest);
	copy_mac_addr(pattrib->src, &etherhdr.h_source);
	...
	if (check_fwstate(pmlmepriv, WIFI_ADHOC_STATE) ||
		...
		copy_mac_addr(pattrib->ra, pattrib->dst);
		copy_mac_addr(pattrib->ta, get_bssid(pmlmepriv));
	}
...
_func_enter_;
	...
	if(pattrib->encrypt ==_TKIP_)//if(psecuritypriv->dot11PrivacyAlgrthm==_TKIP_PRIVACY_)
	{
		...
		if(stainfo!= NULL){
			...
			if(bmcst)
			{
				if(!memchr_inv(psecuritypriv->dot118021XGrptxmickey[psecuritypriv->dot118021XGrpKeyid].skey, 0, 16)){
			...
			{
				if(!memchr_inv(&stainfo->dot11tkiptxmickey.skey[0],0, 16)){
					//DbgPrint("\nxmitframe_addmic:stainfo->dot11tkiptxmickey==0\n");
					//rtw_msleep_os(10);
					return _FAIL;
				}
			...
			for(curfragnum=0;curfragnum<pattrib->nr_frags;curfragnum++){
				payload=(u8 *)_RND4((SIZE_PTR)(payload));
		...
		if (check_fwstate(pmlmepriv,  WIFI_STATION_STATE)) {
			...
			if((ptdlsinfo->setup_state == TDLS_LINKED_STATE)){
				...
				if((ptdls_sta!=NULL)&&(ptdls_sta->tdls_sta_state & TDLS_LINKED_STATE)&&(pattrib->ether_type!=0x0806)){
					...
					copy_mac_addr(pwlanhdr->addr1, pattrib->dst);
					copy_mac_addr(pwlanhdr->addr2, pattrib->src);
					copy_mac_addr(pwlanhdr->addr3, get_bssid(pmlmepriv));
					direct_link=1;
				}else{
					// 1.Data transfer to AP
					// 2.Arp pkt will relayed by AP
					SetToDs(fctrl);
					copy_mac_addr(pwlanhdr->addr1, get_bssid(pmlmepriv));
					copy_mac_addr(pwlanhdr->addr2, pattrib->src);
					copy_mac_addr(pwlanhdr->addr3, pattrib->dst);
				}
			}else
#endif //CONFIG_TDLS
			{
			...
			SetFrDs(fctrl);
			copy_mac_addr(pwlanhdr->addr1, pattrib->dst);
			copy_mac_addr(pwlanhdr->addr2, get_bssid(pmlmepriv));
			copy_mac_addr(pwlanhdr->addr3, pattrib->src);
...
_func_enter_;
	...
	switch(action){
		...
		case TDLS_DISCOVERY_REQUEST:	//unicast: directly to peer sta, Bcast: via AP
			if(mac_addr_equal(pattrib->dst, baddr) )
			{
				SetToDs(fctrl);
				copy_mac_addr(pwlanhdr->addr1, get_bssid(pmlmepriv));
				copy_mac_addr(pwlanhdr->addr2, pattrib->src);
...
_func_enter_;
	...
	while (1)
	{
		...
		mem_start = (unsigned char *)_RND4(addr) + hw_hdr_offset;
...
_func_enter_;
	...
	if (h_proto == 0x8137 || h_proto == 0x80f3)
		RTW_WN24(snap->oui, P802_1H_OUI);
	else
		RTW_WN24(snap->oui, RFC1042_OUI);
...
int rtw_br_client_tx(_adapter *padapter, struct sk_buff **pskb)
{
	...
	//if(check_fwstate(pmlmepriv, WIFI_STATION_STATE|WIFI_ADHOC_STATE))
	{
		...
		if (	!(skb->data[0] & 1) &&
				br_port &&
				!mac_addr_equal(skb->data+ETH_ALEN, padapter->br_mac) &&
				*((unsigned short *)(skb->data+ETH_ALEN*2)) != __constant_htons(ETH_P_8021Q) &&
				*((unsigned short *)(skb->data+ETH_ALEN*2)) == __constant_htons(ETH_P_IP) &&
				mac_addr_equal(padapter->scdb_mac, skb->data+ETH_ALEN) && padapter->scdb_entry) {
			copy_mac_addr(skb->data+ETH_ALEN, GET_MY_HWADDR(padapter));
		...
		{
//			if (priv->dev->br_port &&
//				 mac_addr_equal(skb->data+ETH_ALEN, priv->br_mac)) {
#if 1
			if (*((unsigned short *)(skb->data+ETH_ALEN*2)) == __constant_htons(ETH_P_8021Q)) {
				is_vlan_tag = 1;
				vlan_hdr = *((unsigned short *)(skb->data+ETH_ALEN*2+2));
				memmove(skb->data+ETH_ALEN*2+2, skb->data+ETH_ALEN*2-2, 6 * 2);
				skb_pull(skb, 4);
			}
			//if SA == br_mac && skb== IP  => copy SIP to br_ip ?? why
			if (mac_addr_equal(skb->data+ETH_ALEN, padapter->br_mac) &&
				(*((unsigned short *)(skb->data+ETH_ALEN*2)) == __constant_htons(ETH_P_IP)))
				RTW_WN32(padapter->br_ip, RTW_RN32(skb->data+WLAN_ETHHDR_LEN+12));

			if (*((unsigned short *)(skb->data+ETH_ALEN*2)) == __constant_htons(ETH_P_IP)) {
				if (!mac_addr_equal(padapter->scdb_mac, skb->data+ETH_ALEN)) {
					void *scdb_findEntry(_adapter *priv, unsigned char *macAddr, unsigned char *ipAddr);

					if ((padapter->scdb_entry = (struct nat25_network_db_entry *)scdb_findEntry(padapter,
								skb->data+ETH_ALEN, skb->data+WLAN_ETHHDR_LEN+12)) != NULL) {
						copy_mac_addr(padapter->scdb_mac, skb->data+ETH_ALEN);
						RTW_WN32(padapter->scdb_ip, RTW_RN32(skb->data+WLAN_ETHHDR_LEN+12));
						padapter->scdb_entry->ageing_timer = jiffies;
						do_nat25 = 0;
					}
				}
					...
					if (padapter->scdb_entry) {
						...
						set_zero_mac_addr(padapter->scdb_mac);
						RTW_WN32(padapter->scdb_ip, 0);
					}
				}
			}
			_exit_critical_bh(&padapter->br_ext_lock, &irqL);
			...
			if (do_nat25)
			{
				...
				if (nat25_db_handle(padapter, skb, NAT25_CHECK) == 0) {
					...
					if (is_vlan_tag) {
						skb_push(skb, 4);
						memmove(skb->data, skb->data + 4, 6 * 2);
						*((unsigned short *)(skb->data+ETH_ALEN*2)) = __constant_htons(ETH_P_8021Q);
		...
		if (!mac_addr_equal(skb->data+ETH_ALEN, GET_MY_HWADDR(padapter))) {
			//priv->ext_stats.tx_drops++;
...
sint xmitframe_enqueue_for_sleeping_sta(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	if(bmcst)
	{
		...
		if(pstapriv->sta_dz_bitmap)//if anyone sta is in ps mode
		{
			...
			update_beacon(padapter, _TIM_IE_, 0, _FALSE);//tx bc/mc packets after upate bcn
...
int rtw_ack_tx_wait(struct xmit_priv *pxmitpriv, u32 timeout_ms)
{
	...
	do {
		...
		if (check_c2hcmd != 0)
		{
			...
			else if (ccx_id_filter(check_ccx & 0x0F))
			{
